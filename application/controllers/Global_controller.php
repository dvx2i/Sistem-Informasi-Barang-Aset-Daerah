...
class Global_controller extends CI_Controller
{
	...
	public function validasi($kode_jenis)
	{
		...
		// if ($tanggal_validasi >= $tgl_pengajuan) {
			...
			if ($locked < 1) {
				...
				$data['result'] = $this->global_model->set_validasi($kode_jenis, $id_kib, $tgl_pengajuan);
	...
	public function check_all($kode_jenis)
	{
		...
		foreach($list_id as $key => $value){
			...
			if($value != ''){
			$kib = $this->global_model->get_kib($kode_jenis, $value);
			...
			$tgl_pengajuan = date('Y-m-d', strtotime($kib->tanggal_transaksi));
				...
				if ($locked < 1) {
					$data['result'] = $this->global_model->set_validasi($kode_jenis, $value, $kib->tanggal_transaksi);
					$this->global_model->_logs($menu = 'validasi', $sub_menu = $jenis['controller_name'], $tabel_name = $jenis['table'], $action_id = $value, $action = 'update', $value);
	...
	public function check_all_atb($kode_jenis)
	{
		$kode_jenis = '5.03';
		$jenis = $this->global_model->kode_jenis[$kode_jenis];
		$list_id = $this->input->post('list_id');
		...
		for($i = 0; $i<count($id); $i++){
			...
			$tgl_pengajuan = date('Y-m-d', strtotime($kib->tanggal_transaksi));
				...
				if ($locked < 1) {
					$data['result'] = $this->global_model->set_validasi_atb($kode_jenis, $id[$i], $kib->tanggal_transaksi);
	...
	public function reject_all($kode_jenis = null)
	{
		$jenis = $this->global_model->kode_jenis[$kode_jenis];
		$id_kib = $this->input->post('id_kib');
		$note = $this->input->post('note');

		foreach($id_kib as $key => $value){
			$data = array(
				'validasi' => '3',
				'reject_note' => $note,
				'updated_at' => date('Y-m-d H:i:s'),
			);
			$this->global_model->update_kib($kode_gol = $kode_jenis, $value, $data);
			$this->global_model->_logs($menu = 'validasi', $sub_menu = $jenis['controller_name'], $tabel_name = $jenis['table'], $action_id = $value, $action = 'update', $data, $feature = 'reject');
			//($menu='location', $sub_menu='set_location', $tabel_name='tbl_kode_location', $action_id=$session->id_user, $action='set', $data=$data, $feature);
		}
	...
	public function delete_all($kode_jenis = null)
	{
		// print_r($_POST); die;
		...
		$jenis = $this->global_model->kode_jenis[$kode_jenis];
		  ...
		  foreach ($_POST['list_id'] as $key => $value) {
			...
			{
				$this->load->model('kib_f/Kib_f_model');
			...
			if ($row) {
			  ...
			  $this->global_model->insert_histori_kib($kode_jenis, $id, 'delete_kib'); 
			  $this->global_model->_logs($menu = 'validasi', $sub_menu = $jenis['controller_name'], $tabel_name = $jenis['table'], $action_id = $id, $action = 'delete', $data = $row, $feature = 'delete');
	...
	public function cek_stock_opname_kode_lokasi()
	{
		$bulan = $this->input->post('bulan');
		$tahun = $this->input->post('tahun');
		$id_kode_lokasi =$this->input->post('id_kode_lokasi');
		$id_pengguna =$this->input->post('id_pengguna');

		if($id_kode_lokasi == ''){
			$id_kode_lokasi = $id_pengguna;
		}
