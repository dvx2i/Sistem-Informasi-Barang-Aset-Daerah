...
if (!function_exists('get_action_stock_opname')) {
  function get_action_stock_opname($role = null, $locked_admin = null, $locked_skpd = null, $id = null)
  {
    ...
    } else if ($locked_admin == '0' && $locked_skpd == '1') {
...
if (!function_exists('pengajuan_action')) {
  function pengajuan_action($status_validasi, $id_mutasi)
  {
    ...
    if ($status_validasi <> "2") {
      $delete = anchor(base_url('mutasi/pengajuan/delete/' . $id_mutasi), '<button title="Hapus" class="btn btn-danger"><span class="fa fa-trash" ></span></button>', 'onclick="javasciprt: return confirm(\'Yakin dihapus ?\')"');
    }
    $detail = anchor(base_url('mutasi/pengajuan/detail/' . $id_mutasi), '<button title="Detail" class="btn btn-info"><span class="fa fa-search" ></span></button>');
...
*/
    ...
    $return = '<h4 class="text-success" style="margin:0px;"> Menungu ' . $status_mutasi['deskripsi'] . '</h4>';
    if ($status_mutasi['id_status_mutasi'] == 5) {
      $return = '<h4 class="text-success" style="margin:0px;">' . $status_mutasi['deskripsi'] . '</h4>';
    }
    return $return;
  }
}
...
if (!function_exists('pengecekan_barang_action')) {
  function pengecekan_barang_action($status_proses, $status_validasi, $id_mutasi)
  {
    ...
    */
    if ($status_proses == '2') {
      $cek .=
        anchor(base_url('mutasi/pengecekan_barang/detail/' . $id_mutasi), '<button title="Detail" class="btn btn-info"><span class="fa fa-search"></span></button>') .
        anchor(base_url('mutasi/pengecekan_barang/form_pengecekan/' . $id_mutasi), '&nbsp<button title="Cek" class="btn btn-warning"><span class="fa fa-check-square-o"></span></button>') . '<br>';
      // anchor(base_url('mutasi/pengecekan_barang/delete/' . $id_mutasi), '&nbsp<button title="Hapus" class="btn btn-danger"><span class="fa fa-trash"><span></button>') . '<br>';
    } elseif ($status_proses == '3') {
      $cek .=
        anchor(base_url('mutasi/pengecekan_barang/detail/' . $id_mutasi), '<button title="Detail" class="btn btn-info"><span class="fa fa-search"></span></button>') .
        anchor(base_url('mutasi/pengecekan_barang/form_pengecekan/' . $id_mutasi), '&nbsp<button title="Cek" class="btn btn-warning"><span class="fa fa-check-square-o"></span></button>') .
        // anchor(base_url('mutasi/pengecekan_barang/delete/' . $id_mutasi), '&nbsp<button title="Hapus" class="btn btn-danger"><span class="fa fa-trash"><span></button>') .
        anchor(base_url('mutasi/pengecekan_barang/bast/' . $id_mutasi), '&nbsp<button title="Cetak BAST" class="btn btn-info"><span class="fa fa-file"><span></button>')  .
        anchor(base_url('mutasi/pengecekan_barang/form_pengajuan_register/' . $id_mutasi), '&nbsp<button title="Upload BAST" class="btn btn-success"><span class="fa fa-send"><span></button>') . '<br>';
