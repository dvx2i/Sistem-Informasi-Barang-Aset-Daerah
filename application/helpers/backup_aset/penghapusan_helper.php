...
if ( ! function_exists('pengajuan_action'))
{
  ...
  {
    $return='';
    $hapus = anchor(base_url('penghapusan/pengajuan/delete/'.$id_penghapusan),'<button class="btn btn-danger" title="Hapus"><span class="fa fa-trash"></span></button>','onclick="javasciprt: return confirm(\'Yakin dihapus ?\')"');
    $detail = anchor(base_url('penghapusan/pengajuan/detail/'.$id_penghapusan),'<button class="btn btn-info" title="Detail"><span class="fa fa-search"></span></button>');
    $upload = anchor(base_url('penghapusan/pengajuan/cetak/'.$id_penghapusan),'<button class="btn btn-success" title="Cetak BA Penarikan"><span class="fa fa-file"></span></button>');
    if ($status_proses=="1") {
...
if ( ! function_exists('laporan_action'))
{
  ...
  {
    $return='';
    $detail = anchor(base_url('penghapusan/laporan/detail/'.$id_pengahapusan_lampiran),'<button class="btn btn-info" title="Detail"><span class="fa fa-search"></span></button>');
    $lampiran = anchor(base_url('penghapusan/laporan/create/'.$id_penghapusan),'<button class="btn btn-success" title="Buat Lampiran"><span class="fa fa-file"></span></button>');
...
if ( ! function_exists('lampiran_action'))
{
  ...
  {
    $return='';
    $detail = '<a data-toggle="modal" data-id="'.$id_penghapusan.'" data-paket="'.$nama_paket.'" class="btn btn-info detail" title="Detail"><span class="fa fa-search"></span></a>';
...
if ( ! function_exists('pengecekan_barang_action')){
  function pengecekan_barang_action($id_penghapusan, $status_proses){
    $cek=anchor(base_url('penghapusan/pengecekan_barang/form_pengecekan/'.$id_penghapusan),'<button title="Cek Barang" class="btn btn-warning"><span class="fa fa-check-square-o"><span></button>');
    $detail=anchor(base_url('penghapusan/pengecekan_barang/detail/'.$id_penghapusan),'<button title="Detail" class="btn btn-info" ><span class="fa fa-search"></span></button>');
    $hapus=anchor(base_url('penghapusan/pengecekan_barang/delete/'.$id_penghapusan),'<button title="Hapus" class="btn btn-danger" ><span class="fa fa-trash"></span></button>');
    $return='';
    if ($status_proses=='1') {
...
if ( ! function_exists('penghapusan_barang_action')){
  function penghapusan_barang_action($id_pengahapusan_lampiran){
    $detail = anchor(base_url('penghapusan/penghapusan_barang/detail/'.$id_pengahapusan_lampiran),"<button title='Detail' class='btn btn-info'><span class='fa fa-search'></span></button>");
    $validasi = "<a href='javascript:' id_penghapusan_lampiran='".$id_pengahapusan_lampiran."' class='validasi_penghapusan'><button title='Validasi' class='btn btn-success'><span class='fa fa-check'></span></button></a>";
    return $detail.'&nbsp'.$validasi;
  }
}
