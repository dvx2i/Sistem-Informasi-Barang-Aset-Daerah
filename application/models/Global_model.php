...
public $kode_jenis_ruang = array(
  ...
  {
    $this->db->select('id_kode_barang,kode_barang,nama_barang');
    $this->db->from('tbl_kode_barang');
    $this->db->where('kode_sub_sub_rincian_objek NOT IN ("*","000")');
    $this->db->where('kode_jenis', '02');
    $this->db->order_by('id_kode_barang', 'ASC');
  ...
  public function get_request_validasi()
  {
    ...
    $this->db->join('tbl_master_jenis B', 'CONVERT(A.kode_jenis,char) = CONVERT(B.kode_jenis,char)', 'left');
    $this->db->join('tbl_kode_lokasi C', 'CONVERT(A.id_kode_lokasi,char) = CONVERT(C.id_kode_lokasi,char)', 'left');
    $this->db->where('CONVERT(validasi,signed)', '1');
    $this->db->where('CONVERT(status_barang,char)', 'kib');
  ...
  public function get_request_validasi_a()
  {
    ...
    if($session->id_role !='1'){
      ...
      // $groupby = 'GROUP by kode_jenis'; 
        ...
        left join  tbl_master_jenis B on CONVERT(A.kode_jenis,char) = CONVERT(B.kode_jenis,char) 
		 left join  tbl_kode_lokasi C on CONVERT(A.id_kode_lokasi,char) = CONVERT(C.id_kode_lokasi,char) 
		where CONVERT(A.validasi,signed) < 2  and CONVERT(A.status_barang,char) = 'kib' 
		$where_lokasi 
  ...
  function get_last_stockopname($id_kode_lokasi){
    $sql = "SELECT CONCAT(tahun,'-',bulan,'-01') tgl_stockopname FROM tbl_stock_opname WHERE id_kode_lokasi = '".$id_kode_lokasi."' AND locked_admin > 0 ORDER BY tahun,bulan DESC LIMIT 1";
  ...
  public function cek_hak_akses($id_menu)
  {
	  //session_destroy(); 
	  //session_start();
	  //print_r($this->session->userdata('session')->id_user);die;
	  $this->db->select('A.id_user');
	$this->db->where('A.id_user', $this->session->userdata('session')->id_user);
    //$this->db->join('tbl_role B','A.id_role=B.id_role','left');
   $dapat = $this->db->get('tbl_user A')->row();
   if($dapat){
    ...
    if (!in_array($id_menu, explode(',', $list_id_menu))) {
      redirect(base_url('dashboard'));
	   ...
	   echo '<script type="text/javascript">
		alert("Mohon maaf akun anda sedang dalam kondisi nonaktif sementara  , !!!! data OPD anda dalam proses Rekon !!!!.");
  ...
  function get_ruang_by_lokasi($id_kode_lokasi = null)
  {
   // $this->db->where('kode_kelompok', '3');
    $this->db->where('id_kode_lokasi', $id_kode_lokasi);
    return 
    $this->db->get('tbl_ruang')->result();
  }
