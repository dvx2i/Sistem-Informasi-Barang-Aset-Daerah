...
class Global_kapitalisasi_model extends CI_Model
{
  public $order = 'DESC';
  public $tbl_kapitalisasi = 'tbl_kapitalisasi';
  ...
  function json_kib_a($pihak=null) {
      ...
      $this->datatables->select('id_kib_a,A.nama_barang,A.kode_barang,nomor_register,
        ...
        penggunaan,asal_usul,harga,keterangan,kode_lokasi');
      $this->datatables->from($kib['table'].' A');
      $this->datatables->join($this->tbl_kode_barang.' B', 'A.kode_barang = B.kode_barang');
      $this->datatables->join($this->tbl_kapitalisasi.' C', 'C.kode_jenis="'.$id_jenis.'" and A.'.$kib['id_name'].' = C.id_kib and C.status = 1', 'left');
      ...
      if ($pihak == 'list_barang') {
        ...
        $this->datatables->where('ifnull(C.id_kapitalisasi,0)','0',false);// yang di tampilkan yang belum di mutasi
        ...
        if (!empty($data_reklas)) {
          $this->datatables->where('A.id_kode_barang', $data_reklas['id_kode_barang_tujuan']);
        }
  ...
  function json_kib_b($pihak = null)
  {
    ...
    $this->datatables->select('id_kib_b,A.kode_barang,A.nama_barang,nomor_register,merk_type,
      ...
      nomor_bpkb,asal_usul,harga,keterangan,kode_lokasi, B.nama_barang as nama_barang2,id_inventaris
      ');
  ...
  function json_kib_c($pihak = null)
  {
    ...
    $this->datatables->select('id_kib_c,A.nama_barang,A.kode_barang,nomor_register,
      ...
      gedung_tanggal,gedung_nomor,luas_m2,A.status,nomor_kode_tanah,asal_usul,harga,keterangan,
