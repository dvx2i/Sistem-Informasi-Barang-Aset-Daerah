...
class Laporan extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('31'); //penghapusan laporan
  ...
  public function index()
  {
    ...
    $data['content'] = $this->load->view('laporan/list', $data, true);
  ...
  public function detail($id_pengahapusan_lampiran)
  {
    $data['css'] = array(
      'assets/datatables/dataTables.bootstrap.css',
      'assets/datatables/dataTables.checkbox.css'
    );

    $data['js'] = array(
      'assets/datatables/jquery.dataTables.js',
      'assets/datatables/dataTables.bootstrap.js',
      'assets/datatables/dataTables.checkbox.js'
    );

    $data['id_pengahapusan_lampiran'] = $id_pengahapusan_lampiran;
    $data['content'] = $this->load->view('laporan/detail', $data, true);
    $data['breadcrumb'] = array(
      array('label' => 'Penghapusan', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'Lampiran', 'url' => '#', 'icon' => '', 'li_class' => ''),
      array('label' => 'Detail', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
    );
    $this->load->view('template', $data);
  }
  ...
  public function json_list()
  {
    ...
    echo $this->laporan_model->json_list();
  }

  public function json_detail($id_pengahapusan_lampiran)
  {
    header('Content-Type: application/json');
    echo $this->laporan_model->json_detail($id_pengahapusan_lampiran);
  }
  
  public function json_paket() {
    header('Content-Type: application/json');
    echo $this->laporan_model->json_paket();
  }

  public function create($id_penghapusan = null)
  {
    $data = array(
      'button' => 'Buat',
      'action' => base_url('penghapusan/laporan/create_action'),
      'tanggal_lampiran' => set_value('tanggal_lampiran'),
      'keterangan' => set_value('keterangan'),
      'id_penghapusan' => set_value('id_penghapusan', $id_penghapusan),
    );
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $data = array(
        "tanggal_lampiran" => date('Y-m-d', tgl_inter($this->input->post('tanggal_lampiran', TRUE))),
        "keterangan" => $this->input->post('keterangan', TRUE),
      );

      $insert_id = $this->laporan_model->insert($data);
      $this->global_model->_logs($menu = 'penghapusan', $sub_menu = 'laporan', $tabel_name = 'tbl_penghapusan_lampiran', $action_id = null, $action = 'insert', $data = $data, $feature = 'create_action');
      
      if (!empty($this->input->post('lampiran'))) {
        foreach ($this->input->post('lampiran') as $key) {
          $barang = $this->laporan_model->get_barang($key);

          foreach($barang as $brg){
            $data = array(
              "id_penghapusan_lampiran" => $insert_id,
              "id_penghapusan_barang" => $brg->id_penghapusan_barang,
            );

            $this->laporan_model->insert_detail($data);
            // print_r($data);
            // die;
          }
          $this->pengecekan_barang_model->update_penghapusan($key, array('status_proses' => '3', ));
          // $this->laporan_model->update($insert_id, array('nomor_pembuatan' => $insert_id, ));
          $this->global_model->_logs($menu = 'penghapusan', $sub_menu = 'laporan', $tabel_name = 'tbl_penghapusan_lampiran_detail', $action_id = null, $action = 'insert', $data = $data, $feature = 'create_action');

        }
      }
      // die;
      $this->session->set_flashdata('message', 'Berhasil membuat lampiran.');
      redirect(base_url('penghapusan/laporan'));
    }
  }
  ...
  {
    $this->form_validation->set_rules('tanggal_lampiran', 'tanggal lampiran', 'trim|required');
    $this->form_validation->set_rules('keterangan', 'keterangan', 'trim|required');

    $this->form_validation->set_rules('id_penghapusan_lampiran', 'id penghapusan_lampiran', 'trim');
  ...
  public function laporan_detail($id_pengahapusan_lampiran)
  {
    ...
    $field_name = array("no", "nama_barang", "merk_type", "tanggal_pembelian", "kode_barang", "nomor_register", "asal_usul", "harga_barang", "akumulasi_penyusutan", "nilai_buku","keterangan", "skpd", "instansi");
    $header_label = array('No', 'Nama Barang', 'Merk / tipe', 'Tanggal Beli', 'No. Kode Barang', 'No. Register', 'Asal Usul', 'Harga Barang (Rp.)', 'Akumulasi Penyusutan(Rp.)', 'Nilai Buku(Rp.)', 'Keterangan', 'Nama SKPD', 'Nama Lokasi');
    ...
    $lampiran = $this->laporan_model->get_lampiran($id_pengahapusan_lampiran);
  ...
  public function laporan_rekap($id_pengahapusan_lampiran)
  {
    ...
    $counter++;
    $lampiran = $this->laporan_model->get_lampiran_rekap($id_pengahapusan_lampiran);
  ...
  public function toExcel_20_nov_2018($id_pengahapusan_lampiran)
  {
    ...
    $counter = 2;
    $data['kode_jenis'] = '1';
    $result_1 = $this->laporan_model->get_lampiran($data);
    if ($result_1) {
    ...
    $data['kode_jenis'] = '2';
    $result_2 = $this->laporan_model->get_lampiran($data);
    if ($result_2) {
    ...
    $data['kode_jenis'] = '3';
    $result_3 = $this->laporan_model->get_lampiran($data);
    if ($result_3) {
    ...
    $data['kode_jenis'] = '4';
    $result_4 = $this->laporan_model->get_lampiran($data);
    if ($result_4) {
    ...
    $data['kode_jenis'] = '5';
    $result_5 = $this->laporan_model->get_lampiran($data);
    if ($result_5) {
    ...
    $data['kode_jenis'] = '6';
    $result_6 = $this->laporan_model->get_lampiran($data);
    if ($result_6) {
