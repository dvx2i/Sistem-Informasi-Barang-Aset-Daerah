...
class Validasi_model extends CI_Model
{
  ...
  function set_histori_barang($status_histori, $id_kode_lokasi, $kode_jenis, $id_kib, $tanggal_validasi)
  {
    if($kode_jenis != '5.03'){
      $query = $this->db->query("call proc_histori_barang('" . $status_histori . "'," . $id_kode_lokasi . ",'" . $kode_jenis . "','" . $id_kib ."','" . $tanggal_validasi . "','validasi_kapitalisasi');");
    }else{
      $query = $this->db->query("call proc_histori_barang_atb('" . $status_histori . "'," . $id_kode_lokasi . ",'" . $kode_jenis . "','" . $id_kib ."','" . $tanggal_validasi . "','validasi_kapitalisasi');");
    }
    // $query->next_result();
    // $query->free_result();
    // die($this->db->last_query());
