...
class Kib_b_model extends CI_Model
{
  ...
  { //die(explode('.',$this->session->userdata('session')->kode_lokasi)[4]);
    ...
    $this->datatables->select('(case when (kib_f="1") then "KIB B" else "KIB F" end) as jenis, B.nama_barang, A.nama_barang_migrasi,
      A.id_kib_b,A.kode_barang, A.nama_barang as nama_barang_desc,
      A.nomor_register,A.merk_type,A.ukuran_cc,A.bahan,A.tahun_pembelian,A.nomor_polisi,A.asal_usul,A.harga,A.keterangan,A.kode_lokasi,
       A.nama_barang as nama_barang2,A.validasi, C.instansi,reject_note,COALESCE(A.id_inventaris,A.id_kib_b) as id_inventaris,A.validasi AS status_validasi  ');
