    ...
    class Kib extends CI_Controller
    {
      ...
      public function excel()
      {
        ...
        if ($data['kode_jenis'] == '01') {
          $data['field_name'] = array('no', 'nama_barang_desc', 'kode_barang', 'nomor_register', 'luas', 'tahun_pengadaan', 'letak_alamat', 'status_hak', 'sertifikat_tanggal', 'sertifikat_nomor', 'penggunaan', 'asal_usul', 'harga', 'kondisi', 'keterangan');
          $data['label'] = array("No", "Nama Barang / Uraian", "Kode Barang", "Nomor Register", "Luas (M2)", "Tahun Pengadaan", "Letak Alamat", "Status Hak", "Sertifikat Tanggal", "Sertifikat Nomor", "Penggunaan", "Asal Usul", "Harga", 'Kondisi', "Keterangan");
          if($data['id_kuasa_pengguna'] == ''){
            array_push($data['field_name'], 'instansi');
          }
        } else if ($data['kode_jenis'] == '02') {
          $data['field_name'] = array('no', 'kode_barang', 'nama_barang_desc', 'nomor_register', 'merk_type', 'ukuran_cc', 'bahan', 'tahun_pembelian', 'nomor_pabrik', 'nomor_rangka', 'nomor_mesin', 'nomor_polisi', 'nomor_bpkb', 'asal_usul', 'harga', 'kondisi', 'keterangan');
          $data['label'] = array("No", "Kode Barang", "Nama Barang / Uraian", "Nomor Register", "Merk Type", "Ukuran (CC)", "Bahan", "Tahun Pembelian", "Nomor Pabrik", "Nomor Rangka", "Nomor Mesin", "Nomor Polisi", "Nomor Bpkb", "Asal Usul", "Harga", 'Kondisi', "Keterangan");
          if($data['id_kuasa_pengguna'] == ''){
            array_push($data['field_name'], 'instansi');
          }
        } else if ($data['kode_jenis'] == '03') {
          $data['field_name'] = array('no', 'nama_barang_desc', 'kode_barang', 'nomor_register', 'kondisi_bangunan', 'bangunan_bertingkat', 'bangunan_beton', 'luas_lantai_m2', 'lokasi_alamat', 'gedung_tanggal', 'gedung_nomor', 'luas_m2', 'status', 'nomor_kode_tanah', 'asal_usul', 'harga', 'keterangan');
          ...
          $data['field_name'] = array('no', 'nama_barang_desc', 'kode_barang', 'nomor_register', 'konstruksi', 'panjang_km', 'lebar_m', 'luas_m2', 'letak_lokasi', 'dokumen_tanggal', 'dokumen_nomor', 'status_tanah', 'kode_tanah', 'asal_usul', 'harga', 'kondisi', 'keterangan');
          ...
          $data['field_name'] = array('no', 'nama_barang_desc', 'kode_barang', 'nomor_register', 'judul_pencipta', 'spesifikasi', 'kesenian_asal_daerah', 'kesenian_pencipta', 'kesenian_bahan', 'hewan_tumbuhan_jenis', 'hewan_tumbuhan_ukuran', 'jumlah', 'tahun_pembelian', 'asal_usul', 'harga', 'kondisi', 'keterangan');
          $data['label'] = array('No', 'Nama Barang / Uraian', 'Kode Barang', 'Nomor Register', 'Judul Pencipta', 'Spesifikasi', 'Kesenian Asal Daerah', 'Kesenian Pencipta', 'Kesenian Bahan', 'Hewan Tumbuhan Jenis', 'Hewan Tumbuhan Ukuran', 'Jumlah', 'Tahun Pembelian', 'Asal Usul', 'Harga', 'Keterangan', 'Kondisi');
          if($data['id_kuasa_pengguna'] == ''){
            array_push($data['field_name'], 'instansi');
          }
        } else if ($data['kode_jenis'] == '06') {
          $data['field_name'] = array('no', 'nama_barang_desc', 'bangunan', 'kontruksi_bertingkat', 'kontruksi_beton', 'luas_m2', 'lokasi_alamat', 'dokumen_tanggal', 'dokumen_nomor', 'tanggal_mulai', 'status_tanah', 'nomor_kode_tanah', 'asal_usul', 'nilai_kontrak', 'keterangan'); //,'kode_barang'
          $data['label'] = array('No', 'Nama Barang / Uraian', 'Bangunan', 'Konstruksi Bertingkat', 'Konstruksi Beton', 'Luas (M2)', 'Lokasi Alamat', 'Dokumen Tanggal', 'Dokumen Nomor', 'Tanggal Mulai', 'Status Tanah', 'Nomor Kode Tanah', 'Asal Usul', 'Nilai Kontrak', 'Keterangan'); //,'Kode Barang'
          if($data['id_kuasa_pengguna'] == ''){
            array_push($data['field_name'], 'instansi');
          }
        } else if ($data['kode_jenis'] == '5.03') {
          $data['field_name'] = array('no', 'kode_barang', 'nama_barang_desc',    'nomor_register', 'judul_kajian_nama_software', 'tahun_perolehan', 'asal_usul', 'harga', 'keterangan'); //,'kode_barang'
          $data['label'] =      array('No', 'Kode Barang', 'Jenis / Nama Barang', 'Nomor Register', 'Judul Kajian/Nama Software', 'Tahun Perolehan', 'Asal Usul Cara Perolehan', 'Harga (Rp.)', 'Keterangan'); //,'Kode Barang'
          if($data['id_kuasa_pengguna'] == ''){
            array_push($data['field_name'], 'instansi');
          }
        } else if ($data['kode_jenis'] == '5.02' or $data['kode_jenis'] == '5.04') {
          $data['field_name'] = array('no',  'kode_barang',  'nomor_register',  'nama_barang',  'merk_type',  'sertifikat_nomor',  'bahan', 'asal_usul', 'tanggal_perolehan', 'ukuran_barang', 'satuan', 'keadaan_barang', 'jumlah_barang', 'harga', 'keterangan');
          $data['label'] = array('No',  'Kode Barang',  'Nomor Register',  'Nama Barang / Uraian',  'Merk / Type', 'Sertifikat Nomor', 'Bahan', 'Asal/Cara Perolehan Barang', 'Tahun Perolehan', 'Ukuran Barang / konstruksi (P,S,D)', 'Satuan', 'Keadaan Barang(B,RR,RB)', 'Jumlah Barang', 'Harga', 'Keterangan', 'Unit Kerja');
          if($data['id_kuasa_pengguna'] == ''){
            array_push($data['field_name'], 'instansi');
          }
        }
      ...
      public function toExcel($data = null)
      {
        ...
        if ($data['kode_jenis'] == '01') {
          ...
          if (in_array('instansi', $data['field_name'])) {
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $counter, 'Unit Kerja');
          }
