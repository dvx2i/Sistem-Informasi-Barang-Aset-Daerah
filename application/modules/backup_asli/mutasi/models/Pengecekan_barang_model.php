...
class Pengecekan_barang_model extends CI_Model
{
  ...
  function json()
  {
    ...
    $this->datatables->select('A.id_mutasi,DATE_FORMAT(tanggal, "%d %M %Y") as tanggal,id_kode_lokasi_lama,id_kode_lokasi_baru, status_validasi, A.status_proses,created_at,updated_at, B.instansi as pengguna_lama, C.instansi as pengguna_baru');
  ...
  function get_lampiran_kib($id_mutasi)
  {
    ...
    $this->db->select(' (@no := @no + 1)  as no , 
      ...
      A.keadaan_barang, 1 as jumlah_barang, C.instansi,D.value ');
    $this->db->from(' view_mutasi A ');
