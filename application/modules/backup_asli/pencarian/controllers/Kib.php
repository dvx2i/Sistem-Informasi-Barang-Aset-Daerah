...
class Kib extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $data['kuasa_pengguna'] = null;
    $data['sub_kuasa_pengguna_list'] = null;
    $data['sub_kuasa_pengguna'] = null;

    if ($data['pengguna']) {
      $data['kuasa_pengguna_list'] = $this->pencarian_model->get_kuasa_pengguna($data['pengguna']->id_pengguna);
      ...
      if ($data['kuasa_pengguna']) {
        $data['sub_kuasa_pengguna_list'] = $this->pencarian_model->get_sub_kuasa_pengguna($data['kuasa_pengguna']->id_pengguna, $data['kuasa_pengguna']->id_kuasa_pengguna);
    ...
    // die(json_encode($data['kuasa_pengguna_list']));

    // LEWAT JS SETELAH MEMILIH KODE JENIS
    // // objek
  ...
  public function json_a()
  {
    header('Content-Type: application/json');
    echo $this->Kib_model->json_a();
  }
  public function json_b()
  {
    header('Content-Type: application/json');
    echo $this->Kib_model->json_b();
  }
  public function json_c()
  {
    header('Content-Type: application/json');
    echo $this->Kib_model->json_c();
  }
  public function json_d()
  {
    header('Content-Type: application/json');
    echo $this->Kib_model->json_d();
  }
  public function json_e()
  {
    header('Content-Type: application/json');
    echo $this->Kib_model->json_e();
  }
  public function json_f()
  {
    header('Content-Type: application/json');
    echo $this->Kib_model->json_f();
  }
...
// print_r($_POST); die;
    ...
    if ($data['kode_jenis'] == '01') {
      ...
      $data['field_name'] = array('no', 'nama_barang', 'bangunan', 'bangunan_bertingkat', 'bangunan_beton', 'luas_m2', 'letak_lokasi_alamat', 'dokumen_tanggal', 'dokumen_nomor', 'tgl_bln_thn_mulai', 'status_tanah', 'nomor_kode_tanah', 'asal_usul_pembiayaan', 'nilai_kontrak_rp', 'keterangan', 'instansi',); //,'kode_barang'
      $data['label'] = array('No', 'Nama Barang / Uraian', 'Bangunan', 'Bangunan Bertingkat', 'Bangunan Beton', 'Luas (M2)', 'Lokasi Alamat', 'Dokumen Tanggal', 'Dokumen Nomor', 'Tanggal Mulai', 'Status Tanah', 'Nomor Kode Tanah', 'Asal Usul', 'Nilai Kontrak', 'Keterangan', "Unit Kerja",); //,'Kode Barang'
    }
    ...
    // }
    //tertanda
    $tertanda = $this->pencarian_model->get_tertanda($id_kode_lokasi);
    ...
    $result_lokasi = $this->pencarian_model->get_lokasi_by_id($id_kode_lokasi);
  ...
  public function toExcel($data = null)
  {
    ...
    $kib = $this->Kib_model->get_all($data); //die(json_encode($kib));
