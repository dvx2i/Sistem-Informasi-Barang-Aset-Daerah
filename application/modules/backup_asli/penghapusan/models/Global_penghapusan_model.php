...
class Global_penghapusan_model extends CI_Model
{
  public $order = 'DESC';
  ...
  function json_kib_a($pihak = null)
  {
    ...
    $data = $this->input->post('list_id_kib_a');
    ...
    $this->datatables->select('id_kib_a, A.nama_barang,A.kode_barang,nomor_register,
        ...
        C.id_penghapusan_barang,C.id_penghapusan, C.status_diterima,id_inventaris,deskripsi');
   ...
   $this->datatables->join($this->tbl_kode_barang . ' B', 'A.id_kode_barang = B.id_kode_barang');
    ...
    if ($pihak == 'list_barang') {
      ...
      $this->datatables->where('A.status_barang', 'kib');
      if($this->input->post('jenis') == 'al'){
        $this->datatables->where('B.kode_kelompok', '5', false);
       }
       if ($data) {
      ...
      $this->datatables->where('A.status_barang', 'pengajuan_penghapusan');
    ...
    $this->datatables->where('C.id_penghapusan', $id_penghapusan);
  ...
  function json_kib_b($pihak = null)
  {
	 ...
	 // die(json_encode($pihak));
   $this->load->helper('my_datatable');
   $id_jenis = '02';
   $kib = $this->kib[$id_jenis];
   $data = $this->input->post('list_id_kib_b');
   ...
   $this->datatables->join($this->tbl_kode_barang . ' B', 'A.id_kode_barang = B.id_kode_barang', 'left');
   $this->datatables->join($this->tbl_penghapusan_barang . ' C', ' A.' . $kib['id_name'] . ' = C.id_kib ', 'left');
   // $this->datatables->join($this->tbl_penghapusan . ' D', 'D.id_penghapusan = C.id_penghapusan', 'left');
  $this->datatables->where('A.validasi', '2');
  //$this->datatables->where('A.validasi', '2');
   ...
   if ($pihak == 'list_barang') {
    $this->datatables->where('A.status_barang', 'kib');
     ...
     if($this->input->post('jenis') == 'k'){
      $this->datatables->where('SUBSTR(A.kode_barang,1,12) IN ("1.3.02.01.02","1.3.02.01.01","1.3.02.02.02","1.3.02.02.01")', NULL, FALSE);
    }
     if($this->input->post('jenis') == 'nk'){
      $this->datatables->where('B.kode_kelompok', '3', false);
      $this->datatables->where('SUBSTR(A.kode_barang,1,12) NOT IN ("1.3.02.01.02","1.3.02.01.01","1.3.02.02.02","1.3.02.02.01")', NULL, FALSE);
     }
  ...
  function json_kib_c($pihak = null)
  {
    ...
    $id_jenis = '03';
    $id_penghapusan = $this->input->post('id_penghapusan');
    $data = $this->input->post('list_id_kib_c');
    $kib = $this->kib[$id_jenis];
  ...
  function json_kib_d($pihak = null)
  {
    ...
    $id_jenis = '04';
    $id_penghapusan = $this->input->post('id_penghapusan');
    $data = $this->input->post('list_id_kib_d');
    $kib = $this->kib[$id_jenis];
    $this->datatables->select('id_kib_d, A.nama_barang,B.kode_barang,nomor_register,konstruksi,
      panjang_km,lebar_m,luas_m2,letak_lokasi,dokumen_tanggal,dokumen_nomor,status_tanah,
      kode_tanah,asal_usul,harga,kondisi,keterangan,B.nama_barang as nama_barang2,kode_lokasi,validasi,
      C.id_penghapusan_barang,C.id_penghapusan, C.status_diterima,id_inventaris,deskripsi');
    $this->datatables->from($kib['table'] . ' A');
     $this->datatables->join($this->tbl_kode_barang . ' B', 'A.id_kode_barang = B.id_kode_barang');
    $this->datatables->join($this->tbl_penghapusan_barang . ' C', ' C.kode_jenis="' . $id_jenis . '" and A.' . $kib['id_name'] . ' = C.id_kib ', 'left');
  ...
  function json_kib_e($pihak = null)
  {
    ...
    $id_jenis = '05';
    $id_penghapusan = $this->input->post('id_penghapusan');
    $data = $this->input->post('list_id_kib_e');
    $kib = $this->kib[$id_jenis];
  ...
  function json_kib_f($pihak=null) {
    ...
    $id_jenis='06';
    $id_penghapusan = $this->input->post('id_penghapusan');
    $data = $this->input->post('list_id_kib_f');
    $kib = $this->kib[$id_jenis];
    $this->datatables->select('id_kib_f,A.kode_barang, A.nama_barang,bangunan,kontruksi_bertingkat,
  ...
  function json_kib_atb($pihak=null) {
    $this->load->helper('my_datatable');
    $id_jenis='5.03';
    $id_penghapusan = $this->input->post('id_penghapusan');
    $data = $this->input->post('list_id_kib_atb');
    $kib = $this->kib[$id_jenis];
    $this->datatables->select('id_kib_atb, id_pemilik, A.id_kode_barang, A.id_kode_lokasi, A.nama_barang, A.nama_barang_migrasi, 
      ...
      id_sumber_dana, id_rekening, validasi, reject_note, A.status_barang, id_inventaris,
      C.id_penghapusan_barang,C.id_penghapusan, C.status_diterima,id_inventaris,deskripsi');
    $this->datatables->from($kib['table'].' A');
     $this->datatables->join($this->tbl_kode_barang . ' B', 'A.id_kode_barang = B.id_kode_barang');
    $this->datatables->join($this->tbl_penghapusan_barang . ' C', ' C.kode_jenis="' . $id_jenis . '" and A.' . $kib['id_name'] . ' = C.id_kib ', 'left');
  ...
  {
    $this->db->select('A.id_penghapusan, A.tanggal_pengajuan, B.instansi, A.nama_paket');
