    ...
    $tanggal_kapitalisasi = $tgl_pengajuan;
      ...
      if ($locked < 1) {
        ...
        if ($row->jenis == 'koreksi_tambah') {
          ...
          if($row->kode_jenis == '06'){
            $total = $data_kib->nilai_kontrak + $row->nilai_kapitalisasi;
            $this->global_model->update_kib($row->kode_jenis, $row->id_kib, array('nilai_kontrak' => $total, 'id_sumber_dana_kapitalisasi' => $row->id_sumber_dana_kapitalisasi, 'id_rekening_kapitalisasi' => $row->id_rekening_kapitalisasi));
          }else{
          ...
          $this->validasi_model->set_histori_barang('koreksi_tambah', $row->id_kode_lokasi, $row->kode_jenis, $row->id_kib,$tgl_pengajuan,$row->id_sumber_dana_kapitalisasi, $row->id_rekening_kapitalisasi,$id_kapitalisasi);
          $update_umur = $this->validasi_model->set_umur_ekonomis($row->kode_jenis, $row->id_kib);
          $data_update['umur'] = $update_umur;
          ...
          $this->validasi_model->set_histori_barang('koreksi_kurang', $row->id_kode_lokasi, $row->kode_jenis, $row->id_kib,$tgl_pengajuan,$row->id_sumber_dana_kapitalisasi, $row->id_rekening_kapitalisasi,$id_kapitalisasi);
        } else {
          $data_kib = $this->global_model->get_kib($row->kode_jenis, $row->id_kib);
          if($row->kode_jenis == '06'){
            $total = $data_kib->nilai_kontrak + $row->nilai_kapitalisasi;
          ...
          $this->validasi_model->set_histori_barang('kapitalisasi', $row->id_kode_lokasi, $row->kode_jenis, $row->id_kib,$tgl_pengajuan,$row->id_sumber_dana_kapitalisasi, $row->id_rekening_kapitalisasi,$id_kapitalisasi);
          $update_umur = $this->validasi_model->set_umur_ekonomis($row->kode_jenis, $row->id_kib);
          $data_update['umur'] = $update_umur;
        }
