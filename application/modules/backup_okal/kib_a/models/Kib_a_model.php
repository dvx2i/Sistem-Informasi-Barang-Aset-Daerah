...
class Kib_a_model extends CI_Model
{
  ...
  function json()
  {
    ...
    $this->datatables->select('(case when (kib_f="1") then "KIB A" else "KIB F" end) as jenis, 
      A.id_kib_a, A.nama_barang as nama_barang_desc,
      A.nama_barang, A.nama_barang_migrasi, A.kode_barang,A.nomor_register,A.luas,A.tahun_pengadaan,
      A.letak_alamat,A.status_hak,A.sertifikat_tanggal,A.sertifikat_nomor,A.penggunaan,A.asal_usul,
      A.harga,A.keterangan,A.kode_lokasi,A.created_at,A.updated_at, B.nama_barang as nama_barang2,
      A.validasi, C.instansi,reject_note,COALESCE(A.id_inventaris,A.id_kib_a) as id_inventaris,A.validasi as status_validasi,A.deskripsi ');
  ...
  function delete_histori_barang($id)
  {
    ...
    $this->db->where('kode_jenis', '01'); // kib a
  ...
  {
    $this->db->where('id_kib_a', $id);
    $this->db->delete('tbl_kib_a_histori');
  }
  ...
  {
    $this->load->helper('my_datatable');
    $this->datatables->select('id_kib_a,A.nama_barang,A.kode_barang,nomor_register,luas,tahun_pengadaan,letak_alamat,status_hak,sertifikat_tanggal,sertifikat_nomor,penggunaan,asal_usul,harga,keterangan,kode_lokasi,created_at,updated_at, B.nama_barang as nama_barang2');
    $this->datatables->from('tbl_kib_a A');
    //add this line for join
    //$this->datatables->join('table2', 'tbl_kib_a.field = table2.field');
    $this->datatables->join($this->tbl_barang . ' B', 'A.id_kode_barang = B.id_kode_barang');
    $this->datatables->edit_column('luas', '$1', 'format_number(luas)');
    $this->datatables->edit_column('sertifikat_tanggal', '$1', 'date_time(sertifikat_tanggal)');
    $this->datatables->edit_column('harga', '$1', 'format_float(harga)');
    $this->datatables->edit_column('kode_barang', '$1($2)', 'kode_barang, nama_barang2');
    $checkbox = "
        <label class='switch'>
          <input type='checkbox' class='live' status='1'>
          <span class='slider round'></span>
        </label>
        ";
    $this->datatables->add_column('action', $checkbox);
