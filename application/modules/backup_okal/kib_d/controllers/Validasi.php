...
class Validasi extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('15');
  ...
  public function index()
  {
    ...
    $config['base_url'] = base_url() . 'kib_d/validasi/index/';
    // $config['total_rows'] = $jumlah_data;
    $config['per_page'] = 10;
    $from = $this->uri->segment(4);
    $this->pagination->initialize($config);
    $data['skpd'] = $this->Validasi_model->data($config['per_page'], $from);
    $data["content"] = $this->load->view('kib_d/validasi_skpd', $data, TRUE);
  ...
  public function detail($id_kode_lokasi)
  {
    ...
    $data["content"] = $this->load->view('kib_d/validasi', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Validasi', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
  ...
  public function read($id)
  {
    ...
    if ($row) {
      ...
      $data['content'] = $this->load->view('kib_d/validasi_read', $data, TRUE);
      $data['breadcrumb'] = array(
