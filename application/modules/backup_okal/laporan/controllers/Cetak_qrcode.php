...
class Cetak_qrcode extends MX_Controller
{
  public function __construct()
  {
    ...
    $this->load->model('master_data/Kode_lokasi_model', 'Kode_lokasi_model');
...
echo $this->Kib_b_model->json();
  ...
  public function index()
  {
   $data = array(
  'button' => 'Buat',
  'action' => base_url('laporan/cetak_qrcode/cetak'),
...
$data['content'] = $this->load->view('qrcode/home', $data, true);
...
public function cetak()
  ...
  //  print_r($this->input->post());die;
    ...
    $session = $this->session->userdata('session');
    if ($session->id_upik == 'JSS-A0198') {
      $kib = $this->Kib_b_model->get_all();
      // print_r($kib); die;
      foreach ($kib as $row) {
        // die($row->id_kib_b);
        ...
        array_push(
          ...
          array(
            ...
            "id_kib" => $row->id_kib_e,
            "nama_barang" => !empty($row->nama_barang) ? substr($row->nama_barang, 0, 40) : '-',
            "nomor_register" => !empty($row->nomor_register) ? $row->nomor_register : '-',
            "tahun" => !empty($row->tahun_pembelian) ? $row->tahun_pembelian : '-',
            "spesifikasi" => !empty($row->judul_pencipta) ? substr($row->judul_pencipta, 0, 70)  : '-',
            "lokasi" => $row->akronim,
            "kode_barang" =>  $row->kode_barang,
            "id_enkripsi" => encrypt_url($kode_jenis . $value_2),
            "url_qr" => "https://aset.jogjakota.go.id/scanekir/prev?id=" . encrypt_url($kode_jenis . $value_2),
            "jenis_kib" => $kode_jenis,
            "ruang" => $row->nama_gedung . '-' . $row->lantai . '-' . $row->nama_ruang,
          )
        );

        $params['data'] = "https://aset.jogjakota.go.id/scanekir/prev?id="  . encrypt_url($kode_jenis . $value_2);
        $params['level'] = 'm';
        $params['size'] = 2;
      ...
      if (!empty($list_id_b)) {
        ...
        foreach ($list_id_b as $key => $value_x) {
          $row = $this->Kib_b_model->get_by_id($value_x);
          // echo  $value_x.' aaa '.str_replace("-", ".", $key).' bbb '.encrypt_url(str_replace("-", ".", $key).$value_x);die;
          $value_2 = $row->id_kib_b . "-" . $row->kode_barang;
          array_push(
            $arr_barang,
            array(
              "kode_jenis" => $kode_jenis, // index array menggunkan "." tidak mau
              ...
              "nomor_register" => !empty($row->nomor_register) ? $row->nomor_register : '-',
              "tahun" => !empty($row->tahun_pembelian) ? $row->tahun_pembelian : '-',
              "spesifikasi" => !empty($row->merk_type) ? substr($row->merk_type, 0, 70) : '-',
              "lokasi" => $row->akronim,
              "kode_barang" =>  $row->kode_barang,
              "id_enkripsi" => encrypt_url($kode_jenis . $value_2),
      ...
      if (!empty($list_id_e)) {
        ...
        foreach ($list_id_e as $key => $value_x) {
          $row = $this->Kib_e_model->get_by_id($value_x);
          // echo  $value_x.' aaa '.str_replace("-", ".", $key).' bbb '.encrypt_url(str_replace("-", ".", $key).$value_x);die;
          $value_2 = $row->id_kib_e . "-" . $row->kode_barang;
          array_push(
            $arr_barang,
  ...
  //echo site_url('scanekir')."/scanekir/prev?id=".encrypt_url(str_replace("-", ".", $key).$value_x);die;
...
*/
        $this->load->view('laporan/qrcode/qrcode',array('result'=>$arr_barang));
  } 
