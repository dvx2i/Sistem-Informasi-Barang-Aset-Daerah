...
class Realisasi extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('107');
  ...
  public function index()
  {
    ...
    $data['content'] = $this->load->view('/realisasi/home', $data, true);
  ...
  public function ndownload()
  {
    ...
    if($data['jenis'] == 'bulan'){
      $data['bulan'] = date('n', tgl_inter("1 " . $this->input->post('bulan_tahun')));
      $data['tahun'] = date('Y', tgl_inter("1 " . $this->input->post('bulan_tahun')));
      $data['start_bulan'] = date('n', tgl_inter("1 " . $this->input->post('bulan_tahun')));
      $data['start_tahun'] = date('Y', tgl_inter("1 " . $this->input->post('bulan_tahun')));
      $data['last_bulan'] =  date('n', tgl_inter("1 " . $this->input->post('last_bulan_tahun')));
      $data['last_tahun'] = date('Y', tgl_inter("1 " . $this->input->post('last_bulan_tahun')));
      $data['start_date'] = date('Y-m-d', tgl_inter("1 " . $this->input->post('bulan_tahun')));
      $data['last_date'] = date('Y-m-t', tgl_inter("1 " . $this->input->post('last_bulan_tahun')));
      
//print_r($data['tahun']);die;
     ...
     $data['tahun'] = date('Y');
      if($this->input->post('semester') == '1'){
        ...
        $data['start_date'] = $data['start_tahun'].'-01-01';
        $data['last_date'] = $data['start_tahun'].'-06-30';
      }else{
        $data['start_bulan'] = '7';
        $data['start_tahun'] = date('Y');
        $data['last_bulan'] = '12';
        $data['last_tahun'] = date('Y');
        $data['start_date'] = $data['start_tahun'].'-07-01';
        $data['last_date'] =  $data['start_tahun'].'-12-31';
      }
//print_r($data['tahun']);die;
    }else{
      $data['bulan'] = '';
    ...
    $data['pengguna'] = $lokasi_explode->pengguna;
    /*
...
if($data['id_pengguna'] == '123'){
  $data['mutasi'] = $this->realisasi_model->get_all_rsud($data);
}else{
    $data['mutasi'] = $this->realisasi_model->get_all($data);
}
     //print_r($data['mutasi']); die;
    if ($data['format'] == 'pdf'){
      $this->toPdf($data);
    }
  ...
  public function toExcel($data = null)
  {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
      ->setCellValue('A' . $counter, 'KERTAS KERJA ASET TETAP');
    ...
    $objPHPExcel->setActiveSheetIndex(0)
      ...
      ->setCellValue('A5', 'Bulan/Tahun')
      ->setCellValue('D5', ': '.$data['bulan'].'/'.$data['tahun'])
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $counter, 'KODE REKENING');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $counter, 'URAIAN BELANJA');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $counter, 'NILAI DPA');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $counter, 'REALISASI BELANJA');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $counter, 'REALISASI SIMBADA');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $counter, 'SELISIH');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $counter, 'KETERANGAN');

    $objPHPExcel->getActiveSheet()->getStyle("A" . $first_row_header . ":H7")
      ->applyFromArray($table_header)
      ->getFont()->setSize(12);
      $objPHPExcel->getActiveSheet()->getStyle("A7:H7")
    ...
    // $mutasi = $this->realisasi_model->get_all($data);
    ...
    foreach ($data['mutasi'] as $key => $value1) :
      $counter = $counter + 1;
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $counter, $value1['kode_rekening']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $counter, $value1['nama_rekening']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $counter, $value1['dpa']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $counter, $value1['nilai_realisasi']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $counter, $value1['realisasi']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $counter, '=(E'.$counter.'-F'.$counter.')');
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $counter, '=SUM(D8:D'.$last.')');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $counter, '=SUM(E8:E'.$last.')');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $counter, '=SUM(F8:F'.$last.')');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $counter, '=SUM(G8:G'.$last.')');
    ...
    $objPHPExcel->getActiveSheet()->getStyle($alphabet[0] . "7:" . $alphabet[$jumlah_field - 1] . $counter)
    ...
    $objPHPExcel->getProperties()->setCreator("KERTAS KERJA")
      ->setLastModifiedBy("KERTAS KERJA")
      ->setTitle("Export KERTAS KERJA")
      ->setSubject("Export KERTAS KERJA To Excel")
    ...
    header('Content-Disposition: attachment;filename="Export KERTAS KERJA ' . date('Y-m-d') . '.xlsx"');
