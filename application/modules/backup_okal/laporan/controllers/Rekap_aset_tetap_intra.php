...
class Rekap_aset_tetap_intra extends MX_Controller
{
  function __construct()
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('42'); //laporan
    $this->load->model('rekap_aset_tetap_intra_model');
  ...
  public function index()
  {
    ...
    $data['content'] = $this->load->view('rekap_aset_tetap_intra/home', $data, true);
  ...
  public function ndownload()
  {
    ...
    if ($data['jenis_rekap'] == 'objek') {
      ...
      $data['label'] = array(
        ...
        'NO',  'OBJEK',  'URAIAN', 'JUMLAH BARANG', 'JUMLAH HARGA', 'NILAI PENYUSUTAN', 'AKUMULASI PENYUSUTAN', 'NILAI BUKU',
      );
    ...
    $data['rekap_penyusutan'] = $this->rekap_aset_tetap_intra_model->get_all($data);
    if ($data['format'] == 'pdf')
      $this->toPdf($data);
  ...
  public function toExcel($data = null)
  {
    ...
    foreach ($data['rekap_penyusutan'] as $value) {
      ...
      if($value['status'] == '1') {
          ...
          $t_penyusutan  += $value['nilai_penyusutan'];
          $t_akumulasi += $value['akumulasi_penyusutan'];
          $t_nilaibuku += $value['nilai_buku'];
      }
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('D' . $counter, $t_jumlah);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E' . $counter, $t_harga, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F' . $counter, $t_penyusutan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('G' . $counter, $t_akumulasi, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('H' . $counter, $t_nilaibuku, PHPExcel_Cell_DataType::TYPE_NUMERIC);
  ...
  public function toPdf($data = null)
  {
    ...
    // $data['penyusutan'] = $this->rekap_aset_tetap_intra_model->get_all($data);
