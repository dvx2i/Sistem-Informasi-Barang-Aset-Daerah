    ...
    class Rekap_software extends CI_Controller
    {
      ...
      public function index()
      {
        ...
        $data['content'] = $this->load->view('rekap_software/home', $data, true);
        $data['breadcrumb'] = array(
          array('label' => 'Lampiran LKPD', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
          array('label' => 'Software', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
      ...
      public function excel()
      {
        ...
        $data['format'] = $this->input->post('format');
          ...
          $data['label'] =      array('No', 'Kode Barang', 'Jenis / Nama Barang', 'Nomor Register', 'Nama Software', 'Tahun Perolehan', 'Asal Usul Cara Perolehan', 'Harga (Rp.)', 'Keterangan'); //,'Kode Barang'
  ...
  // }
      ...
      public function toExcel($data = null)
      {
        ...
        //header
          ...
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $counter, 'Judul Software/Nama Software');
    ...
    $no = $mulai == 0 ? $mulai +1 : $mulai;
        ...
        foreach ($kib as $value) {
          if($value['LEVEL'] == '1'){
            if($st_perolehan != 0){
              $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $counter, 'Sub Jumlah');
              $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('H' . $counter, $st_perolehan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
            ...
            $ex->setCellValue( "A". $counter, $value['instansi']);
            ...
            foreach ($alphabet as $key => $v_alphabet) {
              // if (array_key_exists($key, $field_name)) {
                if ($field_name[$key] == 'sertifikat_tanggal' or $field_name[$key] == 'gedung_tanggal' or $field_name[$key] == 'dokumen_tanggal' or $field_name[$key] == 'tanggal_mulai' or $field_name[$key] == 'tgl_bln_thn_mulai') {
                  ...
                  $ex->setCellValueExplicit('H' . $counter,  $value[$field_name[$key]], PHPExcel_Cell_DataType::TYPE_NUMERIC);
                  $ex->getStyle('H'.$counter)->getNumberFormat()->setFormatCode('#,##0.00');
                } else {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $counter, 'Jumlah');
        $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('H' . $counter, $t_perolehan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
...
//print_r($field_name
        ...
        $objPHPExcel->getProperties()->setCreator("Software")
          ->setLastModifiedBy("Software")
          ->setTitle("Export Data Software")
          ->setSubject("Export Software To Excel")
          ->setDescription("Doc for Office 2007 XLSX, generated by PHPExcel.")
          ->setKeywords("office 2007 openxml php")
          ->setCategory("PHPExcel");
        $objPHPExcel->getActiveSheet()->setTitle('Data Software');
        ...
        header('Content-Disposition: attachment;filename="Export Data Software ' . date('Y-m-d') . '.xlsx"');
