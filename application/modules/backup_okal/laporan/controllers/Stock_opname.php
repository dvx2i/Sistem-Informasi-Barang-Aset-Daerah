...
class Stock_opname extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('104'); //SEMENTARA DI LOCK (kasih tnd *) SDG DIPAKAI GENERATE PENYUSUTAN 2020
  ...
  public function index()
  {
    ...
    $data['menu'] = 'Stock Opname';
    $data['content'] = $this->load->view('stock_opname/list', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Stock Opname', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'Stock Opname', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
    );
  ...
  public function rekap()
  {
    ...
    $data['show_lokasi'] = true;
    $data['menu'] = 'Stock Opname';
    $data['content'] = $this->load->view('stock_opname/list_rekap', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Rekap Stock Opname', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'Rekap Stock Opname', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
    );
    $this->load->view('template', $data);
  }
  ...
  function json_rekap()
  {
    ...
    $skpd = $this->Stock_opname_model->get_rekap_skpd($tahun);
    $lokasi = $this->Stock_opname_model->get_rekap_lokasi($tahun);
    ...
    if(!empty($skpd)){
      ...
      // }
        $result .= '{
          "data": {
              "SKPD": "'.$a['instansi'].'",
              "Jan": "'.icon_checklist($a['Jan']).'",
              "Feb": "'.icon_checklist($a['Feb']).'",
              "Mar": "'.icon_checklist($a['Mar']).'",
              "Apr": "'.icon_checklist($a['Apr']).'",
              "Mei": "'.icon_checklist($a['Mei']).'",
              "Jun": "'.icon_checklist($a['Jun']).'",
              "Jul": "'.icon_checklist($a['Jul']).'",
              "Agu": "'.icon_checklist($a['Agu']).'",
              "Sep": "'.icon_checklist($a['Sep']).'",
              "Okt": "'.icon_checklist($a['Okt']).'",
              "Nov": "'.icon_checklist($a['Nov']).'",
              "Des": "'.icon_checklist($a['Des']).'"
          } ';

          if(!empty($lokasi)){
            $numItems = count($lokasi);
            $i = 0;
            $result .= ', "kids": [';
            foreach($lokasi as $b){
              if($b['pengguna'] == $a['pengguna']){
                $result .= '
                {
                  "data": {
                    "'.$a['instansi'].'": "'.$b['instansi'].'",
                    "Jan": "'.icon_checklist($b['Jan']).'",
                    "Feb": "'.icon_checklist($b['Feb']).'",
                    "Mar": "'.icon_checklist($b['Mar']).'",
                    "Apr": "'.icon_checklist($b['Apr']).'",
                    "Mei": "'.icon_checklist($b['Mei']).'",
                    "Jun": "'.icon_checklist($b['Jun']).'",
                    "Jul": "'.icon_checklist($b['Jul']).'",
                    "Agu": "'.icon_checklist($b['Agu']).'",
                    "Sep": "'.icon_checklist($b['Sep']).'",
                    "Okt": "'.icon_checklist($b['Okt']).'",
                    "Nov": "'.icon_checklist($b['Nov']).'",
                    "Des": "'.icon_checklist($b['Des']).'"
                },
                    "kids": []
              },';
  ...
  public function create_action()
  {
    ...
    if(is_array($this->input->post('id_kuasa_pengguna'))){
      ...
      $last_data = $this->Stock_opname_model->get_last_data($data);
      ...
      {
       $new_data = $this->Stock_opname_model->insert($data);
       ...
       if ($this->db->trans_status() === FALSE) {
        ...
        echo json_encode(array('status' => false, 'message' => 'Stock Opname Bulan '.bulan_indo($next_month).' Tahun '.$next_year.' Belum Diinput'));
      }
  ...
  */
  ...
  public function create_action()
  {
      ...
      // $lokasi = $this->Stock_opname_model->get_lokasi_by_pengguna($this->input->post('id_pengguna', TRUE));
    ...
    foreach ($lokasi as $key => $value) {
      ...
      $stock_opname = $this->Stock_opname_model->get_by_lokasi($value, $data['bulan'], $data['tahun']);
  ...
  public function lock()
  {
    ...
    $stock_opname = $this->Stock_opname_model->get_by_id($id);

    $locked = $this->Stock_opname_model->get_unlocked($stock_opname->bulan, $stock_opname->tahun, $stock_opname->id_kode_lokasi);
    if ($locked < 1) {
      // die;
      ...
      if ($session->id_role == '1') {
        if($stock_opname->locked_admin == '0'){
          $set_stock_opname = $this->Stock_opname_model->set_stock_opname($data); //lock hitung soalnya sebelumnya kebuka
          
          $data['intra_ekstra'] = '01';
          $set_stock_opname = $this->Stock_opname_model->set_stock_opname($data); // stock opname intra
          
          $data['intra_ekstra'] = '02';
          $set_stock_opname = $this->Stock_opname_model->set_stock_opname($data); //stock opname extra
        }
        $data_update['locked_admin'] = '2'; //lock uperadmin nga perlu hitung ulang
      } else {
        $set_stock_opname = $this->Stock_opname_model->set_stock_opname($data); //lock skpd hitung ulang soalnya sebelumnya kebuka
      ...
      $this->Stock_opname_model->update($id, $data_update);
  ...
  public function lock_skpd($id = null)
  {
    ...
    if ($locked < 1) {
      ...
      $set_stock_opname = $this->Stock_opname_model->set_stock_opname($data); //stock opname all
  ...
  public function unlock()
  {
    ...
    $locked = $this->Stock_opname_model->get_locked($stock_opname->bulan, $stock_opname->tahun, $stock_opname->id_kode_lokasi);
    if ($locked < 1) {
      // die;
