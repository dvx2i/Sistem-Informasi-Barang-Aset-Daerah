...
class Stock_opname_kib extends MX_Controller
{
  ...
  public function index()
  {
    ...
    $data['menu'] = 'Stock Opname KIB';
    $data['content'] = $this->load->view('stock_opname_kib/list', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Stock Opname KIB', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'Stock Opname KIB', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
    );
  ...
  public function json()
  {
    ...
    echo $this->Stock_opname_kib_model->json();
  }
  ...
  public function rekap()
  {
    ...
    $data['show_lokasi'] = true;
    $data['menu'] = 'Stock Opname KIB';
    $data['content'] = $this->load->view('stock_opname_kib/list_rekap', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Rekap Stock Opname KIB', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'Rekap Stock Opname KIB', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
    );
    $this->load->view('template', $data);
  }
  ...
  function json_rekap()
  {
    ...
    $skpd = $this->Stock_opname_kib_model->get_rekap_skpd($tahun);
    $lokasi = $this->Stock_opname_kib_model->get_rekap_lokasi($tahun);
  ...
  public function create_action()
  {
    ...
    if(is_array($this->input->post('id_kuasa_pengguna'))){
      ...
      $last_data = $this->Stock_opname_kib_model->get_last_data($data);
      ...
      {
       $new_data = $this->Stock_opname_kib_model->insert($data);
       ...
       if ($this->db->trans_status() === FALSE) {
        ...
        echo json_encode(array('status' => false, 'message' => 'Stock Opname KIB Bulan '.bulan_indo($next_month).' Tahun '.$next_year.' Belum Diinput'));
      }
  ...
  */
  ...
  public function create_action()
  {
      ...
      // $lokasi = $this->Stock_opname_kib_model->get_lokasi_by_pengguna($this->input->post('id_pengguna', TRUE));
      ...
      $this->db->trans_start();
          ...
          $this->Stock_opname_kib_model->insert($data);

          $this->Stock_opname_kib_model->backup($data);
  ...
  {
    $row = $this->Stock_opname_kib_model->get_by_id($id);
    // bila sudah di validasi dan bukan super admin
    // if ($row->validasi == '2' and $this->session->userdata('session')->id_role != '1')
    if ($row) {

      $this->Stock_opname_kib_model->delete_backup($id);
      $this->Stock_opname_kib_model->delete($id);

      $this->session->set_flashdata('message', 'Delete Record Success');
    } else {
      $this->session->set_flashdata('message', 'Record Not Found');
    }
    redirect(base_url('stock_opname_kib'));
  }
