...
class Kib_b_model extends CI_Model
{
  ...
  { //die(explode('.',$this->session->userdata('session')->kode_lokasi)[4]);
    ...
    $this->datatables->join($this->tbl_kode_lokasi . ' C', 'A.id_kode_lokasi = C.id_kode_lokasi','LEFT');
     $this->datatables->join($this->tbl_ruang_barang . ' D', 'A.id_kib_b = D.id_kib AND "02" = D.kode_jenis', 'LEFT');
   ...
   if($this->input->post('id_ruang') != ''){
        $this->datatables->where('D.id_ruang', $this->input->post('id_ruang')); // where id kuasa pengguna
      }
      if($this->input->post('id_pengguna') != ''){
    $this->datatables->where('C.pengguna', $this->input->post('id_pengguna'));
      }
  ...
  function get_all()
  {
    $this->db->select('A.*,C.*,D.instansi,D.akronim');
    $this->db->from($this->table .' A');
    $this->db->join('tbl_ruang_barang B', 'A.'.$this->id.' = B.id_kib AND "02" = B.kode_jenis', 'LEFT');
    $this->db->join('tbl_ruang C', 'B.id_ruang = C.id_ruang', 'LEFT');
    $this->db->join('tbl_kode_lokasi D', 'A.id_kode_lokasi = D.id_kode_lokasi', 'LEFT');
    $this->db->where('validasi', '2');
    $this->db->where('A.id_pemilik', '2');
    $this->db->where('A.id_kode_barang_aset_lainya', '0');
    $this->db->where_not_in('A.status_barang', array('terhapus'));
    $this->db->where('A.id_kib_b >', '353725');
    $this->db->order_by('A.id_kode_lokasi', 'asc');
    $this->db->order_by('A.id_kode_barang', 'asc');
    $this->db->order_by('A.nomor_register', 'asc');
    $this->db->limit('1');
    // $this->db->offset('300000');
    // return 
    $this->db->get()->result();
    die($this->db->last_query());
  }
