...
class Rekap_mutasi_model extends CI_Model
{
  ...
  function get_all($data)
  {
    ...
    if ($data['jenis_rekap'] == 'objek') {
      // die("call proc_rekap_mutasi_objek(" . $data['session']->id_user . "," . $data['id_pemilik'] . "," . $data['id_kode_lokasi'] . ",'" . $data['start_date'] . "','" . $data['last_date'] . "', '" . $data['intra_ekstra'] . "')");
      $query    = $this->db->query("call proc_rekap_mutasi_objek(" . $data['session']->id_user . "," . $data['id_pemilik'] . "," . $data['id_kode_lokasi'] . ",'" . $data['start_bulan'] . "','" . $data['start_tahun'] . "','" . $data['last_bulan'] . "','" . $data['last_tahun'] . "',  '" . $data['intra_ekstra'] . "')");
      $res      = $query->result_array();
    } else if ($data['jenis_rekap'] == 'rincian_objek') {
      $query    = $this->db->query("call proc_rekap_mutasi_rincian_objek(" . $data['session']->id_user . "," . $data['id_pemilik'] . "," . $data['id_kode_lokasi'] . ",'" . $data['start_bulan'] . "','" . $data['start_tahun'] . "','" . $data['last_bulan'] . "', '" . $data['last_tahun'] . "',  '" . $data['intra_ekstra'] . "')");
      $res      = $query->result_array();
    }
