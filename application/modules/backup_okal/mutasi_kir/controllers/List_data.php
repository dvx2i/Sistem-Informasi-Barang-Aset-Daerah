...
class List_data extends MX_Controller
{
  ...
  public function detail($id_mutasi)
  {
   ...
   // $data['mutasi_picture'] = $this->global_mutasi_model->get_mutasi_picture($id_mutasi);
    $data['list_status_mutasi'] = $this->global_mutasi_model->get_status_mutasi_all_kir();
  ...
  */
  ...
  public function update($id)
  {
    ...
    if ($row && $this->session->userdata('session')->id_role == '1')  {
      $data = array(
        'button' => 'Ajukan',
        'action' => base_url('mutasi_kir/list_data/update_action/' . $id),
        ...
        'id_ruang_baru' => set_value('id_ruang_baru', $row->id_ruang_baru),
        'id_ruang_lama' => set_value('id_ruang_lama', $row->id_ruang_lama),
      );
      $data['css'] = array(
        'assets/datatables/dataTables.bootstrap.css',
        'assets/sweetalert/sweetalert2.css',
      ...
      $data['lokasi'] = $this->global_model->get_all_lokasi();
       $data['list_ruang'] = $this->global_model->get_all_ruang();
      $data['list_status_mutasi'] = $this->global_mutasi_model->get_status_mutasi_all_kir();
...
*/
      $this->global_model->_logs($menu = 'mutasi_kir', $sub_menu = 'list_data', $tabel_name = 'tbl_mutasi_kir', $action_id = $id_mutasi, $action = 'update', $data = array("tanggal" => tgl_inter($this->input->post('tanggal_validasi', TRUE)),), $feature = 'update_action');
      
      $this->session->set_flashdata('message', 'Berhasil Diupdate');
    }
    redirect(base_url('mutasi_kir/list_data'));
  }
}
