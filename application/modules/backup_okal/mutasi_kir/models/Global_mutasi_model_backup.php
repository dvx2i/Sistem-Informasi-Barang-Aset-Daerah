...
class Global_mutasi_model extends CI_Model
{
    ...
    function json_kib_a($pihak = null, $data = null)
    {
      ...
      validasi');
            ...
            $this->datatables->where(" (id_kode_barang_aset_lainya is NULL and A.status_barang <> 'aset_lainya') ", NULL, FALSE);
        ...
        validasi,C.id_mutasi_barang,status_diterima,status_validasi, id_kode_lokasi_lama, id_kode_lokasi_baru');
            $this->datatables->from($this->tbl_mutasi_barang . ' C');
            $this->datatables->join($this->tbl_mutasi . ' D', 'C.id_mutasi = D.id_mutasi', 'left');
            $this->datatables->join($kib['table'] . ' A', ' A.' . $kib['id_name'] . ' = C.id_kib', 'left');
            $this->datatables->join($this->tbl_kode_barang . ' B', 'A.id_kode_barang = B.id_kode_barang');
            // $this->datatables->join($this->tbl_mutasi_barang . ' C', 'C.kode_jenis="' . $id_jenis . '" and A.' . $kib['id_name'] . ' = C.id_kib', 'left');
            // $this->datatables->join($this->tbl_mutasi . ' D', 'D.id_mutasi = C.id_mutasi', 'left');
            $this->datatables->where('A.validasi', '2');
  ...
  function json_kib_f($pihak=null, $data=null) {
    ...
    $this->datatables->select('id_kib_f,kode_barang,nama_barang,bangunan,kontruksi_bertingkat,
      ...
      C.id_mutasi_barang,status_diterima,status_validasi, id_kode_lokasi_lama, id_kode_lokasi_baru');
    $this->datatables->from($kib['table'].' A');
    $this->datatables->join($this->tbl_mutasi_barang.' C', 'C.kode_jenis="'.$id_jenis.'" and A.'.$kib['id_name'].' = C.id_kib', 'left');
    $this->datatables->join($this->tbl_mutasi.' D', 'D.id_mutasi = C.id_mutasi', 'left');
    $this->datatables->where('A.validasi','2');
