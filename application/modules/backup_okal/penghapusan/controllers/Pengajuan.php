...
class Pengajuan extends CI_Controller
{
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      foreach ($this->input->post('pengajuan') as $key => $value) {
        ...
        }elseif($this->input->post('jenis') == 'nk'){
          $paket = 'NON_KENDARAAN_'.$this->global_model->kode_jenis[$kode_jenis]['nama_pendek'].'_'.$nama_lokasi;
        }elseif($this->input->post('jenis') == 'al'){
          $paket = 'ASET_LAINNYA_'.$this->global_model->kode_jenis[$kode_jenis]['nama_pendek'].'_'.$nama_lokasi;
        }else{
  ...
  public function cetak($id_pengahapusan_lampiran)
  {
    ...
    $field_name = array("no","kode_barang",  "nomor_register", "nama_barang", "merk_type", "nomor_nomor","harga_barang", "tahun_perolehan", "kondisi", "status_diterima", "status_diterima", "keterangan", "instansi");
    $header_label = array('No','Kode Barang', 'No. Register', 'Nama Barang', 'Merk / tipe', "No.Sertifikat/ No.Pabrik/ No.Chasis/ No.Mesin/ No.Polisi/ No.BPKB",'Harga', 'Tahun Perolehan', 'Kondisi (B/RR/RB)', 'Ditarik', 'Tidak Ditarik', 'Keterangan', 'Instansi');
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $counter, 'HARGA');
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I' . $counter, 'Kondisi (B/RR/RB)');
    $objPHPExcel->getActiveSheet()->mergeCells('I' . $counter . ':I' . $merge_botom);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $counter, 'Pengecekan *)');
    $objPHPExcel->getActiveSheet()->mergeCells('J' . $counter . ':K' . $counter);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L' . $counter, 'Keterangan');
    $objPHPExcel->getActiveSheet()->mergeCells('L' . $counter . ':L' . $merge_botom);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M' . $counter, 'Lokasi');
    $objPHPExcel->getActiveSheet()->mergeCells('M' . $counter . ':M' . $merge_botom);
    $counter++;
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $counter, 'Ditarik');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K' . $counter, 'Tidak Ditarik');
    $counter++;
    $objPHPExcel->getActiveSheet()->getStyle("A7:M9")
    ->applyFromArray($table_header);
    $objPHPExcel->getActiveSheet()->getStyle("A7:M8")
    ->getAlignment()->setWrapText(true);
    // $objPHPExcel->getActiveSheet()->getRowDimension(1)->setRowHeight(-1);
    $objPHPExcel->getActiveSheet()->getStyle("A5:M5")
    ->getAlignment()->setWrapText(true);
    ...
    foreach ($lampiran as $key => $value) {
      ...
      $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $counter, $value['harga_barang']);
      $objPHPExcel->getActiveSheet()->getStyle('G' . $counter)->getNumberFormat()->setFormatCode('#,##0.00');
      if($value['tahun_perolehan'] != '0'){
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $counter, $value['tahun_perolehan']);
      }else{
      ...
      $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K' . $counter, $value['status_diterima']);
      if(!empty($value['id_inventaris'])){
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L' . $counter, $value['keterangan'].' / '.$value['id_inventaris']);
      }else{
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L' . $counter, $value['keterangan']);
      }
      $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M' . $counter, $value['instansi']);

      $counter++;
    }
    $sum = $counter-1;
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $counter, "=SUM(G10:G".$sum.")");
    $objPHPExcel->getActiveSheet()->getStyle('G' . $counter)->getNumberFormat()->setFormatCode('#,##0.00');
    $counter--;
