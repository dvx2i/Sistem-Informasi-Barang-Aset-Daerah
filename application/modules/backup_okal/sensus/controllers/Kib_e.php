...
class Kib_e extends MX_Controller
{
  ...
  public function index()
  {
    ...
    $data['barang'] = $this->global_model->getBarangE();
    //$session = $this->session->userdata('session');
  ...
  public function update_sensus()
  {
    ...
    $this->db->trans_start();
        ...
        // print_r($data); die;
        $this->Kib_e_model->delete_sensus('05', $value);
        $this->Kib_e_model->insert_sensus($data);
...
//print_r($this->session->userdata('data_reklas'));die;
    $this->load->view('template', $data);
  }
      ...
      if ($row->validasi == 2 && $this->session->userdata('session')->id_role == '1') { // kib yg sudah tervalidasi (hanya admin yg bisa)
        ...
        if ($row->input_banyak > 0) {
          ...
          foreach ($row_kib as $key) {
            $this->global_model->update_histori_barang('05', $key->id_kib_e, $row->tanggal_transaksi, 'entri', $data_histori);
          }
