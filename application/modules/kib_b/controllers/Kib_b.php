...
class Kib_b extends MX_Controller
{
  protected $kode_jenis = '02';
  function __construct()
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('7');
    $this->load->model('Kib_b_model');
  ...
  public function index()
  {
    ...
    $data['content'] = $this->load->view('kib_b/tbl_kib_b_list', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Data KIB', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'KIB B', 'url' => '#', 'icon' => '', 'li_class' => 'active'),
    );
  ...
  public function read($id)
  {
    ...
    if ($row) {
      ...
      $data['content'] = $this->load->view('kib_b/tbl_kib_b_read', $data, TRUE);
      $this->load->view('template', $data);
    } else {
  ...
  public function create()
  {
    ...
    $data = array(
      'button' => 'Buat',
      'menu' => $this->global_model->kode_jenis[$this->kode_jenis]['nama'],
      'action' => base_url('kib_b/create_action'),
      'id_kib_b' => set_value('id_kib_b'),
      'sumber_dana' => set_value('sumber_dana'),
      'rekening' => set_value('rekening'),
      'kode_barang' => set_value('kode_barang'),
      'nama_barang' => set_value('nama_barang'),
      'merk_type' => set_value('merk_type'),
      'ukuran_cc' => set_value('ukuran_cc'),
      'bahan' => set_value('bahan'),
      'tahun_pembelian' => set_value('tahun_pembelian'),
      'nomor_pabrik' => set_value('nomor_pabrik'),
    ...
    // die(json_encode($data['rincian_objek']));
    // sub_rincian_objek
    ...
    // die(json_encode($data));
    $data['content'] = $this->load->view('kib_b/tbl_kib_b_form', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Data KIB', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
      array('label' => 'KIB B', 'url' => '#', 'icon' => '', 'li_class' => ''),
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      // $data['kode_barang_kib_f'] = $this->global_model->get_kode_barang_kib_f('2');
      $data = array(
        ...
        'ukuran_cc' => to_float($this->input->post('ukuran_cc', TRUE)),
        'bahan' => $this->input->post('bahan', TRUE),
        'tahun_pembelian' => date('Y', tgl_inter($this->input->post('tanggal_pembelian', TRUE))),
        'nomor_pabrik' => $this->input->post('nomor_pabrik', TRUE),
      ...
      if ($jumlah_barang > 1) {
        $insert_id = $this->Kib_b_model->insert($data); //ambil id untuk input_banyak
        $this->Kib_b_model->update($insert_id, array('input_banyak' => $insert_id,)); //update insert yang pertama
        ...
        for ($jumlah_barang; $jumlah_barang > 0; $jumlah_barang--) {
          $this->Kib_b_model->insert($data);
          $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_b', $tabel_name = 'tbl_kib_b', $action_id = null, $action = 'insert', $data = $data);
        ...
        $id = $this->Kib_b_model->insert($data);
				$this->global_model->update_reklas($data_reklas['id_reklas_kode'],array('id_kib_tujuan' => $id));
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_b', $tabel_name = 'tbl_kib_b', $action_id = $id, $action = 'insert_reklas', $data = $data);
  ...
  public function update($id)
  {
    ...
    if ($row) {
      ...
      $data = array(
        'button' => 'Perbarui',
        'menu' => $this->global_model->kode_jenis[$this->kode_jenis]['nama'],
        'action' => base_url('kib_b/update_action'),
        'id_kib_b' => set_value('id_kib_b', $row->id_kib_b),
        ...
        'merk_type' => set_value('merk_type', $row->merk_type),
        'ukuran_cc' => set_value('ukuran_cc', $row->ukuran_cc),
        'bahan' => set_value('bahan', $row->bahan),
        'tahun_pembelian' => set_value('tahun_pembelian', $row->tahun_pembelian),
        'nomor_pabrik' => set_value('nomor_pabrik', $row->nomor_pabrik),
  ...
  public function update_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      $this->session->set_flashdata('message', 'Data Tidak Lengkap');
      $this->update($this->input->post('id_kib_b', TRUE));
    } else {

      $row = $this->Kib_b_model->get_by_id($this->input->post('id_kib_b', TRUE));
      ...
      if( $this->input->post('tanggal_pembelian', TRUE) != '-'){
        $data['tahun_pembelian'] = date('Y', tgl_inter($this->input->post('tanggal_pembelian', TRUE)));
      }
      
      // if( $this->input->post('tanggal_transaksi', TRUE) != '-'){
      //   $data['tanggal_transaksi'] = date('Y-m-d', tgl_inter($this->input->post('tanggal_transaksi', TRUE)));
      // }
      ...
      if ($this->input->post('validasi', TRUE) != '2') {
        ...
        if( $this->input->post('tanggal_perolehan', TRUE) != '-'){
          $data['tanggal_perolehan'] = date('Y-m-d', tgl_inter($this->input->post('tanggal_perolehan', TRUE)));
        }

      }

      
      if ($row->validasi == 2 ) { // kib yg sudah tervalidasi 
        $this->global_model->insert_histori_kib('02', $this->input->post('id_kib_b', TRUE), 'update_kib'); // 02 = KIB B
        $data['validasi'] = '2';
        ...
        // if( $this->input->post('tanggal_transaksi', TRUE) != '-'){
        //   $data_histori['tanggal_histori'] = date('Y-m-d', tgl_inter($this->input->post('tanggal_transaksi', TRUE)));
        // }

        if ($row->input_banyak > 0) {

          $row_kib = $this->global_model->get_kib_input_banyak('02', $row->input_banyak);

          foreach ($row_kib as $key) {
            // $this->global_model->update_histori_barang('02', $key->id_kib_b, $data_histori);
            $this->global_model->update_histori_barang('02', $key->id_kib_b, $row->tanggal_transaksi, 'entri', $data_histori);
          }

        } else {
          $this->global_model->update_histori_barang('02', $this->input->post('id_kib_b', TRUE), $row->tanggal_transaksi, 'entri', $data_histori);
        }
      }
      ...
      if (!empty($data_reklas)) {
        //REKLAS CONDITION
        $data['status_barang'] = 'reklas_kode';
        $this->Kib_b_model->update($this->input->post('id_kib_b', TRUE), $data);
        // $this->global_model->global_update(
        //   'tbl_reklas_kode',
        //   array('id_reklas_kode' => $data_reklas['id_reklas_kode']),
        //   array('id_kib_tujuan' => $this->input->post('id_kib_b', TRUE),)
        // );
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_b', $tabel_name = 'tbl_kib_b', $action_id = $this->input->post('id_kib_b', TRUE), $action = 'update_reklas', $data = $data);
        ...
        if($row->input_banyak > 0){
          $this->Kib_b_model->update_input_banyak($row->input_banyak, $data);
        }else{
          $this->Kib_b_model->update($this->input->post('id_kib_b', TRUE), $data);
        }
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_b', $tabel_name = 'tbl_kib_b', $action_id = $this->input->post('id_kib_b', TRUE), $action = 'update', $data = $data);
  ...
  public function delete($id)
  {
    ...
    if ($row) {
      ...
      $this->Kib_b_model->delete($id);
      $this->Kib_b_model->delete_histori_barang($id,'02');
