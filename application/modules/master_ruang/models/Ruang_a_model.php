...
class Ruang_a_model extends CI_Model
{
  ...
  function json()
  {
    ...
    $this->datatables->select('A.id_ruang,A.id_lantai,A.id_pemilik,A.id_kode_lokasi,A.nama_ruang,A.keterangan,A.deskripsi,A.kode_lokasi,A.latitute,A.longitute,A.status_ruang,A.created_at,A.updated_at,A.letak_alamat,A.lantai,A.nama_gedung, C.instansi');
    $this->datatables->from('tbl_ruang A');
   ...
   // $this->datatables->edit_column('harga', '$1', 'format_float(harga)');
    $this->datatables->add_column('action', '$1', 'get_action_ruang(' . $session->id_role . ',master_ruang,'. $this->id .')');
 ...
 /* function delete_histori_kib($id)
  ...
  {
    $this->load->helper('my_datatable');
    $this->datatables->select('id_ruang,A.nama_barang,A.kode_barang,nomor_register,luas,tahun_pengadaan,letak_alamat,status_hak,sertifikat_tanggal,sertifikat_nomor,penggunaan,asal_usul,harga,keterangan,kode_lokasi,created_at,updated_at, B.nama_barang as nama_barang2');
