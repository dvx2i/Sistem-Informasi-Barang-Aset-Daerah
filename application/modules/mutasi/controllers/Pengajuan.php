...
class Pengajuan extends MX_Controller
{
  ...
  public function create()
  {
    $data = array(
      'button' => 'Buat',
      'action' => base_url('mutasi/pengajuan/create_action'),
      'id_mutasi' => set_value('id_usulan'),
      'kib' => set_value('kib'),
      ...
      'kode_lokasi_lama' => set_value('kode_lokasi_lama'),
    ...
    if (!empty($data['id_pengguna'])) {
      $data['kuasa_pengguna_list'] = $this->pengajuan_model->get_kuasa_pengguna($data['id_pengguna']);
      ...
      if (!empty($data['id_kuasa_pengguna'])) {
        $data['sub_kuasa_pengguna_list'] = $this->pengajuan_model->get_sub_kuasa_pengguna($data['id_pengguna'], $data['id_kuasa_pengguna']);
    ...
    if ($data['kode_lokasi_baru'])
      $data['lokasi'] = $this->pengajuan_model->get_lokasi($data['kode_lokasi_baru']);
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $id_lokasi_baru = $this->input->post('kode_lokasi_baru', TRUE);
      ...
      $this->global_model->_logs($menu = 'mutasi', $sub_menu = 'pengajuan', $tabel_name = 'tbl_mutasi_barang', $action_id = null, $action = 'insert', $data = $arr_barang, $feature = 'create_action');
