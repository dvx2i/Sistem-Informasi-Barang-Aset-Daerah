...
class List_data_model extends CI_Model
{
  ...
  function json()
  {
    ...
    $this->datatables->from('tbl_mutasi_kir A');
    ...
    $this->datatables->add_column('detail', '<a href="' . base_url("mutasi_kir/list_data/detail/$1") . '"> <button class="btn btn-info btn-sm" title="Detail"> <span class="fa fa-search"></span></button></a>', 'encrypt_url(id_mutasi)');
    $this->datatables->add_column('action', '<a href="' . base_url("mutasi_kir/list_data/update/$1"). '"> <button class="btn btn-warning btn-sm" title="Update"> <span class="fa fa-edit"></span></button></a>', 'encrypt_url(id_mutasi)');
  ...
  function json_detail($id_mutasi)
  {
    $this->load->helper('my_datatable');
    $this->db->query('SET lc_time_names = "id_ID";');
    $this->datatables->select('A.id_mutasi_barang, A.status_diterima,A.id_kib,B.kib_name, B.nomor_register, C.kode_barang, C.nama_barang,A.status_validasi');
    $this->datatables->from('tbl_mutasi_barang_kir A');
  ...
  function update_url_bast($id_mutasi, $data)
  {
    ...
    $this->db->update('tbl_mutasi_kir', $data);
  }
  ...
  {
    $query = $this->db->query("call proc_mutasi_kir(" . $id_mutasi . ")");
    $res      = $query->result();
