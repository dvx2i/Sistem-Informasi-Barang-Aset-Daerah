...
class Kib extends CI_Controller
{
  ...
  public function excel()
  {
    ...
    $data['start_date'] = !empty($this->input->post('start_date')) ? date('Y-m-d', tgl_inter($this->input->post('start_date'))) : '';
    $data['last_date'] = !empty($this->input->post('last_date')) ? date('Y-m-d', tgl_inter($this->input->post('last_date'))) : '';
...
// }
    ...
    $kode_lokasi = !empty($result_lokasi->kode_lokasi) ? $result_pemilik->kode . '.' . $result_lokasi->kode_lokasi : '2';
    $data['kode_lokasi'] = get_intra_extra(remove_star($kode_lokasi), $data['intra_ekstra']);
    $data['nama_lokasi'] =  !empty($result_lokasi->instansi) ? ' - ' . $result_lokasi->instansi : 'Kota Yogyakarta';
