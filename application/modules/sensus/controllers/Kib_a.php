...
class Kib_a extends MX_Controller
{
  ...
  public function index()
  {
   ...
   $this->session->unset_userdata('data_reklas');
    ...
    $data['barang'] = $this->global_model->getBarangA();
    //$session = $this->session->userdata('session');
    ...
    $data["content"] = $this->load->view('kib_a/tbl_kib_a_list', $data, TRUE);
  ...
  public function update_sensus()
  {
    ...
    $this->db->trans_start();
        ...
        // print_r($data); die;
        $this->Kib_a_model->delete_sensus('01', $value);
        $this->Kib_a_model->insert_sensus($data);
  ...
  {
    $id = decrypt_url($id);
    $row = $this->Kib_a_model->get_by_id($id);
    if ($row) {
      $data = array(
        'menu' => $this->global_model->kode_jenis[$this->kode_jenis]['nama'],
        'id_kib_a' => $row->id_kib_a,
      ...
      $data['content'] = $this->load->view('kib_a/tbl_kib_a_read', $data, TRUE);
      $this->load->view('template', $data);
    } else {
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      if ($jumlah_barang > 1) {
        ...
        $id = $this->Kib_a_model->insert($data);
        $this->global_model->update_reklas($data_reklas['id_reklas_kode'],array('id_kib_tujuan' => $id));

        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_a', $tabel_name = 'tbl_kib_a', $action_id = $id, $action = 'insert_reklas', $data = $data);
  ...
  public function update_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      $this->update($this->input->post('id_kib_a', TRUE));
    } else {
      $data = array(
      ...
      // die(json_encode($data));
      $row = $this->Kib_a_model->get_by_id($this->input->post('id_kib_a', TRUE));

      if ($row->validasi == 2 && $this->session->userdata('session')->id_role == '1') { // kib yg sudah tervalidasi
        $this->global_model->insert_histori_kib('01', $this->input->post('id_kib_a', TRUE), 'update_kib'); // 1 = KIB A
        $data['validasi'] = '2';
        $data['tanggal_perolehan'] = $this->input->post('tanggal_perolehan', TRUE) != '-' ?  date('Y-m-d', tgl_inter($this->input->post('tanggal_perolehan', TRUE))) : '0000-00-00';
        ...
        $this->global_model->update_histori_barang('01', $this->input->post('id_kib_a', TRUE),$row->tanggal_transaksi, 'entri', $data_histori);
      }
      ...
      if (!empty($data_reklas)) {
        //REKLAS CONDITION
        $data['status_barang'] = 'reklas_kode';
        $this->Kib_a_model->update($this->input->post('id_kib_a', TRUE), $data);
        // $this->global_model->global_update(
        //   'tbl_reklas_kode',
        //   array('id_reklas_kode' => $data_reklas['id_reklas_kode']),
        //   array('id_kib_tujuan' => $this->input->post('id_kib_a', TRUE),)
        // );
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_a', $tabel_name = 'tbl_kib_a', $action_id = $this->input->post('id_kib_a', TRUE), $action = 'update_reklas', $data = $data);
        ...
        $this->Kib_a_model->update($this->input->post('id_kib_a', TRUE), $data);
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_a', $tabel_name = 'tbl_kib_a', $action_id = $this->input->post('id_kib_a', TRUE), $action = 'update', $data = $data);
  ...
  public function delete($id)
  {
    ...
    if ($row) {
      ...
      $this->Kib_a_model->delete($id);
      $this->Kib_a_model->delete_histori_barang($id);
      $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_a', $tabel_name = 'tbl_kib_a', $action_id = $id, $action = 'delete', $data = $row);
  ...
  public function _rules()
  {
    ...
    $this->form_validation->set_rules('status_hak', 'status hak', 'trim|required');
    $this->form_validation->set_rules('sertifikat_tanggal', 'sertifikat tanggal', 'trim|required|callback_max_year');
    $this->form_validation->set_rules('sertifikat_nomor', 'sertifikat nomor', 'trim|required');
    ...
    $this->form_validation->set_rules('id_kib_a', 'id_kib_a', 'trim');
