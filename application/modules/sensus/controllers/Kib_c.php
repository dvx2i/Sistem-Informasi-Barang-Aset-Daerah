...
class Kib_c extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('8');
    $this->load->model('Kib_c_model');
  ...
  public function index()
  {
   ...
   $this->session->unset_userdata('data_reklas');
    ...
    $data['barang'] = $this->global_model->getBarangC();
    ...
    $data['content'] = $this->load->view('kib_c/tbl_kib_c_list', $data, TRUE);
  ...
  public function update_sensus()
  {
    ...
    $this->db->trans_start();
        ...
        // print_r($data); die;
        $this->Kib_c_model->delete_sensus('03', $value);
        $this->Kib_c_model->insert_sensus($data);
  ...
  {
    $id = decrypt_url($id);
    $row = $this->Kib_c_model->get_by_id($id);
    if ($row) {
      $data = array(
        'menu' => $this->global_model->kode_jenis[$this->kode_jenis]['nama'],
        'id_kib_c' => $row->id_kib_c,
        'nama_barang' => $row->nama_barang,
        'kode_barang' => $row->kode_barang,
        'nomor_register' => $row->nomor_register,
        'kondisi_bangunan' => $row->kondisi_bangunan,
        'bangunan_bertingkat' => $row->bangunan_bertingkat,
        'bangunan_beton' => $row->bangunan_beton,
        'luas_lantai_m2' => my_format_number($row->luas_lantai_m2),
        'lokasi_alamat' => $row->lokasi_alamat,
        'gedung_tanggal' => tgl_indo($row->gedung_tanggal),
        'gedung_nomor' => $row->gedung_nomor,
      ...
      $data['content'] = $this->load->view('kib_c/tbl_kib_c_read', $data, TRUE);
      $this->load->view('template', $data);
    } else {
  ...
  public function create()
  {
    ...
    $data = array(
      'button' => 'Buat',
      'menu' => $this->global_model->kode_jenis[$this->kode_jenis]['nama'],
      'action' => base_url('kib_c/create_action'),
      'id_kib_c' => set_value('id_kib_c'),
      'sumber_dana' => set_value('sumber_dana'),
      'rekening' => set_value('rekening'),
      'nama_barang' => set_value('nama_barang'),
      'kode_barang' => set_value('kode_barang'),
      // 'nomor_register' => set_value('nomor_register'),
      'kondisi_bangunan' => set_value('kondisi_bangunan'),
      'bangunan_bertingkat' => set_value('bangunan_bertingkat'),
      'bangunan_beton' => set_value('bangunan_beton'),
      'luas_lantai_m2' => set_value('luas_lantai_m2'),
      'lokasi_alamat' => set_value('lokasi_alamat'),
      'gedung_tanggal' => set_value('gedung_tanggal'),
      'gedung_nomor' => set_value('gedung_nomor'),
    ...
    $data['content'] = $this->load->view('kib_c/tbl_kib_c_form', $data, TRUE);
    $data['breadcrumb'] = array(
      array('label' => 'Data KIB', 'url' => '#', 'icon' => 'fa fa-pie-chart', 'li_class' => ''),
  ...
  public function create_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      // print_r($this->global_model->get_kode_lokasi($this->input->post('kode_lokasi', TRUE))); die;
      // $data['kode_barang_kib_f'] = $this->global_model->get_kode_barang_kib_f('3');
      ...
      $data = array(
        ...
        'kondisi_bangunan' => $this->input->post('kondisi_bangunan', TRUE),
        'bangunan_bertingkat' => $this->input->post('bangunan_bertingkat', TRUE),
        'bangunan_beton' => $this->input->post('bangunan_beton', TRUE),
        'luas_lantai_m2' => to_float($this->input->post('luas_lantai_m2', TRUE)),
        'lokasi_alamat' => $this->input->post('lokasi_alamat', TRUE),
        'gedung_tanggal' => date('Y-m-d', tgl_inter($this->input->post('gedung_tanggal', TRUE))),
        'gedung_nomor' => $this->input->post('gedung_nomor', TRUE),
        'luas_m2' => to_float($this->input->post('luas_m2', TRUE)),
      ...
      //REKLAS CONDITION
      $data_reklas = $this->session->userdata('data_reklas');
      // print_r($data_reklas); die;

      $this->db->trans_start();

      if ($jumlah_barang > 1) {
        for ($jumlah_barang; $jumlah_barang > 0; $jumlah_barang--) {
          $this->Kib_c_model->insert($data); //memang di awal , supaya no register pas
          $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_c', $tabel_name = 'tbl_kib_c', $action_id = null, $action = 'insert', $data = $data);
        ...
        $id = $this->Kib_c_model->insert($data);
        // die($id);
        ...
        $this->global_model->global_update(
          ...
          array('id_kib_tujuan' => $id,)
        );
        
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_c', $tabel_name = 'tbl_kib_c', $action_id = $id, $action = 'insert_reklas', $data = $data);
  ...
  public function update_action()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      $this->update($this->input->post('id_kib_c', TRUE));
    } else {
      $data = array(
      ...
      $row = $this->Kib_c_model->get_by_id($this->input->post('id_kib_c', TRUE));
      
      if ($row->validasi == 2 && $this->session->userdata('session')->id_role == '1') { // kib yg sudah tervalidasi
        $this->global_model->insert_histori_kib('03', $this->input->post('id_kib_c', TRUE), 'update_kib'); // 1 = KIB A
        $data['validasi'] = '2';
        $data['tanggal_perolehan'] = $this->input->post('tanggal_perolehan', TRUE) != '-' ?  date('Y-m-d', tgl_inter($this->input->post('tanggal_perolehan', TRUE))) : '0000-00-00';
        ...
        $this->global_model->update_histori_barang('03', $this->input->post('id_kib_c', TRUE), $row->tanggal_transaksi, 'entri', $data_histori);
      ...
      if (!empty($data_reklas)) {
        //REKLAS CONDITION
        $data['status_barang'] = 'reklas_kode';
        $this->Kib_c_model->update($this->input->post('id_kib_c', TRUE), $data);
        // $this->global_model->global_update(
        //   'tbl_reklas_kode',
        //   array('id_reklas_kode' => $data_reklas['id_reklas_kode']),
        //   array('id_kib_tujuan' => $this->input->post('id_kib_c', TRUE),)
        // );
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_c', $tabel_name = 'tbl_kib_c', $action_id = $this->input->post('id_kib_c', TRUE), $action = 'update_reklas', $data = $data);
        ...
        $this->Kib_c_model->update($this->input->post('id_kib_c', TRUE), $data);
        $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_c', $tabel_name = 'tbl_kib_c', $action_id = $this->input->post('id_kib_c', TRUE), $action = 'update', $data = $data);
  ...
  public function delete($id)
  {
    ...
    if ($row) {
      // die;
      $this->db->trans_start();
        $this->global_model->insert_histori_kib('03', $id, 'delete_kib'); // 3 = KIB C

      $this->Kib_c_model->delete($id);
      $this->Kib_c_model->delete_histori_barang($id);
      $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_c', $tabel_name = 'tbl_kib_c', $action_id = $id, $action = 'delete', $data = $row);
    ...
    if(!empty($_POST[ 'list_id'])) {
      ...
      foreach ($_POST['list_id'] as $key => $value) {
        ...
        if ($row) {
          ...
          if ($row->validasi == '2'){
          $this->global_model->insert_histori_kib('03', $id, 'delete_kib'); // 3 = KIB C$this->Kib_c_model->delete_histori_barang($id);
          $this->Kib_c_model->delete_histori_barang($id);
          $this->global_model->_logs($menu = 'data_kib', $sub_menu = 'kib_c', $tabel_name = 'tbl_kib_c', $action_id = $id, $action = 'delete', $data = $row);
          }
  ...
  public function _rules()
  {
    ...
    $this->form_validation->set_rules('kondisi_bangunan', 'kondisi bangunan', 'trim|required');
    $this->form_validation->set_rules('bangunan_bertingkat', 'bangunan bertingkat', 'trim|required');
    $this->form_validation->set_rules('bangunan_beton', 'bangunan beton', 'trim|required');
    $this->form_validation->set_rules('luas_lantai_m2', 'luas lantai m2', 'trim|required');
    $this->form_validation->set_rules('lokasi_alamat', 'lokasi alamat', 'trim|required');
    $this->form_validation->set_rules('gedung_tanggal', 'gedung tanggal', 'trim|required');
    $this->form_validation->set_rules('gedung_nomor', 'gedung nomor', 'trim|required');
    ...
    $this->form_validation->set_rules('id_kib_c', 'id_kib_c', 'trim');
