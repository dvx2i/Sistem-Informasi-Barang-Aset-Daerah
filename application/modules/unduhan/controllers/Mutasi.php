...
class Mutasi extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('72'); //pengecekan barang
    $this->load->model('mutasi/global_mutasi_model');
  ...
  public function index()
  {
    ...
    $data['content'] = $this->load->view('mutasi/list', $data, true);
  ...
  public function json()
  {
    ...
    echo $this->mutasi_model->json();
  }

  public function json_detail($id_mutasi)
  {
    header('Content-Type: application/json');
    echo $this->mutasi_model->json_detail($id_mutasi);
  }
  ...
  public function form_pengecekan($id)
  {
    ...
    if ($row) {
      ...
      $data['jenis_mutasi'] = $this->mutasi_model->get_jenis_mutasi($id)->jenis; //ambil kode jenis yang ada
  ...
  public function pengecekan_barang_action($id_mutasi = null)
  {
    ...
    $this->mutasi_model->update_pengajuan($arr_barang, 'id_mutasi_barang');
    $this->mutasi_model->update_pengajuan_register($id_mutasi, array('status_proses' => '3'));
  ...
  public function form_pengajuan_register($id)
  {
    ...
    if ($row) {
      ...
      $data['list_status_mutasi'] = $this->global_mutasi_model->get_status_mutasi_all();
      $data['id_status_mutasi'] = $this->global_mutasi_model->get_mutasi($id);
      $data['picture_bast'] = $this->mutasi_model->get_picture_bast($id);
  ...
  public function pengajuan_register_action($id_mutasi = null)
  {
    ...
    $row = $this->mutasi_model->get_by_id($id_mutasi);
    ...
    if(!empty($row->tanggal_bast)) {
        ...
        $this->mutasi_model->update_pengajuan_register($id_mutasi, $data_update);
        $this->mutasi_model->update_pengajuan_register($id_mutasi, array('status_proses' => '4'));
...
// die($fileExt);
    ...
    if ($this->upload->do_upload($fieldname)) {
      ...
      $id_mutasi_picture = $this->mutasi_model->insert_picture($data_insert);
  ...
  {
    $this->mutasi_model->remove_picture($id_mutasi_picture);
  ...
  public function bast_action($id_mutasi = null)
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->mutasi_model->update_bast($id_mutasi, $data_update);
  ...
  public function cetak_bast_word($id_mutasi)
  {
    ...
    $data["data"] = "";
    $row   = $this->mutasi_model->get_bast($id_mutasi);
    $barang= $this->mutasi_model->get_jumlah_barang($id_mutasi);
    // $data['jenis'] = $this->global_model->kode_jenis[$data['kode_jenis']];
  ...
  public function toExcel($id_mutasi)
  {
    ...
    $data['field_name'] = array(
      'no',  'kode_barang',  'nomor_register',  'nama_barang',  'merk_type',  'sertifikat_nomor',  'bahan', 'asal_usul', 'tanggal_perolehan', 'ukuran_barang', 'satuan', 'keadaan_barang', 'jumlah_barang', 'harga', 'nilai_penyusutan', 'akumulasi_penyusutan', 'nilai_buku', 'keterangan', 'id_kib', 'instansi'
    );
    $data['label'] = array(
      'No',  'Kode Barang',  'Nomor Register',  'Nama Barang / Uraian',  'Merk / Type', 'Sertifikat Nomor', 'Bahan', 'Asal/Cara Perolehan Barang', 'Tahun Perolehan', 'Ukuran Barang / konstruksi (P,S,D)', 'Satuan', 'Keadaan Barang(B,RR,RB)', 'Jumlah Barang', 'Harga', 'Nilai Penyusutan', 'Akumulasi Penyusutan', 'Nilai Buku', 'Keterangan', 'ID KIB', 'Unit Kerja'
    );
    // die();
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O' . $counter, 'Penyusutan');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R' . $counter, 'Keterangan');
    ...
    $objPHPExcel->getActiveSheet()->mergeCells('O' . $counter . ':Q' . $counter);
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O' . $counter, 'Nilai Penyusutan');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('P' . $counter, 'Akumulasi Penyusutan');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q' . $counter, 'Nilai Buku');
    ...
    $objPHPExcel->getActiveSheet()->getStyle("A" . $first_row_header . ":" . $last_alpabet . $last_row_header)
      ...
      ->getFont()->setSize(12);
      $objPHPExcel->getActiveSheet()->getStyle("A8:T9")
    ...
    $inventaris = $this->mutasi_model->get_lampiran_kib($id_mutasi,$lastPenyusutan);
    ...
    foreach ($field_name as $key => $value) :
      ...
      if ($value == "harga" || $value == "nilai_penyusutan" || $value == "akumulasi_penyusutan" || $value == "nilai_buku") {
