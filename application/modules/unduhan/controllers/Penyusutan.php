...
class Penyusutan extends MX_Controller
{
  ...
  {
    parent::__construct();
    $this->global_model->cek_hak_akses('73'); //laporan
    $this->load->model('Penyusutan_model');
    $this->load->model('penyusutan/laporan_model');
    $this->load->model('laporan/stock_opname_kib_model');
  ...
  public function ndownload()
  {
    ...
    $data['last_bulan'] = date('m', tgl_inter("31 " . $this->input->post('bulan_tahun')));
    $data['last_tahun'] = date('Y', tgl_inter("31 " . $this->input->post('bulan_tahun')));
    ...
    $data['field_name'] = array(
      ...
      'no',  'kode_barang',  'nomor_register',  'nama_barang',  'merk_type', 'tanggal_pembelian', 'umur_bulan', 'umur_ekonomis', 'sisa_umur_ekonomis', 'nilai_perolehan', 'nilai_penyusutan', 'akumulasi_penyusutan','akumulasi_penyusutan_lama', 'nilai_buku', 'instansi',
    ...
    $data['label'] = array(
      ...
      'No',  'Kode Barang',  'Nomor Register',  'Nama Barang / Uraian',  'Merk / Type', 'Tanggal_pembelian/Pengadaan', 'umur_bulan', 'umur_ekonomis', 'Sisa Umur Ekonomis', 'Nilai Perolehan', 'Nilai Penyusutan', 'Akumulasi Penyusutan','Akumulasi Penyusutan Juni 2022', 'Nilai Buku', 'Unit Kerja',
    );
    ...
    $pengguna = isset($result_lokasi->pengguna) ? $result_lokasi->pengguna : 0;

    $backup_kib = $this->stock_opname_kib_model->get_by_lokasi($pengguna,$semester,$data['last_tahun']);
  ...
  public function toExcel($data = null)
  {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I' . $counter, 'Sisa Umur Ekonomis');
    // $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I' . $counter, 'Jumlah Barang');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $counter, 'Nilai Perolehan');
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M' . $counter, 'Akumulasi Penyusutan Juni 2022');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N' . $counter, 'Nilai Buku');
    ...
    foreach ($penyusutan as $value) {
      foreach ($alphabet as $key => $v_alphabet) {
        ...
        // }
        else if (($field_name[$key] == 'umur_bulan' or $field_name[$key] == 'umur_ekonomis' or $field_name[$key] == 'sisa_umur_ekonomis' or $field_name[$key] == 'jumlah_barang' or $field_name[$key] == 'nilai_perolehan' or $field_name[$key] == 'nilai_penyusutan' or
          $field_name[$key] == 'akumulasi_penyusutan' or $field_name[$key] == 'akumulasi_penyusutan_lama' or $field_name[$key] == 'nilai_buku') and ($value[$field_name[$key]] == '0')) {
        ...
        if($field_name[$key] == 'akumulasi_penyusutan_lama'){
          $t_akumulasi_lama += $value[$field_name[$key]];
        }
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('J' . $counter, $t_perolehan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('K' . $counter, $t_penyusutan, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('L' . $counter, $t_akumulasi, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('M' . $counter, $t_akumulasi_lama, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('N' . $counter, $t_nilaibuku, PHPExcel_Cell_DataType::TYPE_NUMERIC);
    ...
    foreach ($alphabet as $columnID) {
      if (in_array($columnID, array('G', 'H', 'I', 'J', 'K', 'L', 'M', 'N')))
    ...
    foreach ($field_name as $key => $value) :
      if (
        ...
        $value == 'akumulasi_penyusutan' or $value == 'nilai_buku' or $value == 'akumulasi_penyusutan_lama'
      ) {
    ...
    $objPHPExcel->getProperties()->setCreator("ASET")
      ...
      ->setTitle("Export Data Koreksi Tanggal Perolehan")
    ...
    header('Content-Disposition: attachment;filename="Export Data Koreksi Tanggal Perolehan ' . date('Y-m-d') . '.xlsx"');
